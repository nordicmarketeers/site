...
export const TransactionPageComponent = props => {
	...
	const handleSubmitOrderRequest = values => {
		...
		const priceVariantNameMaybe = priceVariantName
			? { priceVariantName }
			: {};
	...
	const foundListingTypeConfig = validListingTypes.find(
		conf =>
			conf.listingType === listing?.attributes?.publicData?.listingType
	);
		...
		<TransactionPanel
			...
			activityFeed={
				<ActivityFeed
					...
					onShowOlderMessages={() =>
						onShowMoreMessages(transaction.id, config)
					}
				...
				{showMakeCounterOfferModal ? (
					<MakeCounterOfferModal
						...
						onMakeCounterOffer={onMakeCounterOffer(
							...
							transactionRole === CUSTOMER
								? process?.transitions
										?.CUSTOMER_MAKE_COUNTER_OFFER
								: process?.transitions
										?.PROVIDER_MAKE_COUNTER_OFFER,
							onTransition,
							transactionRole,
							currency,
							setMakeCounterOfferModalOpen,
...
const mapDispatchToProps = dispatch => {
	return {
		...
		onFetchTransactionLineItems: (orderData, listingId, isOwnListing) =>
			dispatch(
				fetchTransactionLineItems(orderData, listingId, isOwnListing)
			), // for OrderPanel
