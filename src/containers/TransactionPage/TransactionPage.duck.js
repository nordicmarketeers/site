	...
	delay(3000)
		...
		.then(response => {
			...
			const lastTransition =
				response?.data?.data?.attributes?.lastTransition;
...
export const fetchTimeSlots = (
	...
	return dispatch(
		fetchTimeSlotsThunk({ listingId, start, end, timeZone, options })
	);
};
...
const makeTransitionPayloadCreator = (
	...
	const transaction = getState()?.marketplaceData?.entities?.transaction?.[
		txId?.uuid
	];
	const processName = resolveLatestProcessName(
		transaction?.attributes?.processName
...
export const sendReview = (
	...
	return dispatch(
		sendReviewThunk({ tx, transitionOptionsInfo, params, config })
	);
};
...
// fetchTransitions //
//////////////////////
const fetchTransitionsPayloadCreator = (
	{ id },
...
export const fetchTransactionLineItems = ({
	...
	return dispatch(
		fetchLineItemsThunk({ orderData, listingId, isOwnListing })
	);
};
	...
	extraReducers: builder => {
		builder
			...
			.addCase(fetchTimeSlotsThunk.pending, (state, action) => {
				...
				if (useFetchTimeSlotsForDate) {
					const dateId = stringifyDateToISO8601(
