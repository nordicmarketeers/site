...
describe("TransactionPage", () => {
	...
	describe('default-purchase process with "item" (Sale)', () => {
		...
		test.each(purchases)(
			"check purchase: $tr",
			async ({ tr, tx, isReversal, isReceived }) => {
				...
				expect(
					screen.getByText("TransactionPage.listingTitleMobile")
				).toBeInTheDocument();
				...
				expect(
					screen.queryAllByText("OrderBreakdown.baseUnitQuantity")
				).toHaveLength(expectedLength);
				expect(
					screen.queryAllByText("OrderBreakdown.subTotal")
				).toHaveLength(expectedLength);
				...
				expect(
					screen.queryAllByText("OrderBreakdown.refund")
				).toHaveLength(expectedLengthReversal);
				expect(screen.queryAllByText("-$10.00")).toHaveLength(
				...
				expect(
					screen.queryAllByText("OrderBreakdown.commission")
				).toHaveLength(expectedLength);
				expect(screen.queryAllByText("-$1.00")).toHaveLength(
					expectedLength
				);

				// Order breakdown: commission refund?
				expect(
					screen.queryAllByText("OrderBreakdown.refundProviderFee")
				).toHaveLength(expectedLengthReversal);
				...
				const expectedTotal = isReversal
					? expectedLengthReversal
					: expectedLength;
				expect(screen.queryAllByText(total)).toHaveLength(
					expectedTotal
				);

				// A note about commission (customer should no see this)
				expect(
					screen.queryAllByText("OrderBreakdown.commissionFeeNote")
				).toHaveLength(expectedLength);

				// Activity feed (here we just check the heading)
				expect(
					screen.getByText("TransactionPanel.activityHeading")
				).toBeInTheDocument();

				// Listing's title (side card)
				expect(
					screen.getByText("listing-item title")
				).toBeInTheDocument();
				...
				const primaryButtonText =
					stateData.primaryButtonProps?.buttonText || "no-button";
				// length == 2 => mobile & desktop are rendered separately
				const expectedLengthPrimary = stateData.primaryButtonProps
					?.buttonText
					? 2
					: 0;
				expect(screen.queryAllByText(primaryButtonText)).toHaveLength(
					expectedLengthPrimary
				);

				// Messaging
				const sendMsg = screen.getByPlaceholderText(
					"TransactionPanel.sendMessagePlaceholder"
				);
				expect(sendMsg).toBeInTheDocument();
				const sendMsgButton = screen.queryByRole("button", {
					name: "SendMessageForm.sendMessage",
				});
	...
	describe('default-purchase process with "item" (Order)', () => {
		...
		test.each(purchases)(
			...
			async ({ tr, tx, isReversal, isReceived }) => {
				...
				expect(
					screen.queryAllByText("OrderBreakdown.subTotal")
				).toHaveLength(isReversal ? expectedLength : 0);
				expect(screen.queryAllByText("$10.00")).toHaveLength(
				...
				expect(
					screen.queryAllByText("listing-item title")
				).toHaveLength(isInquiry ? 0 : 1);
	...
	describe('default-booking process with "day" (Sale)', () => {
		...
		test.each(bookings)(
			...
			async ({ tr, tx, isReversal, isReceived }) => {
				...
				expect(
					screen.queryAllByText("OrderBreakdown.bookingStart")
				).toHaveLength(expectedLength);
				expect(screen.queryAllByText("Tuesday")).toHaveLength(
					expectedLength * 2
				);
				expect(screen.queryAllByText("Jun 14")).toHaveLength(
					expectedLength * 2
				);
				expect(
					screen.queryAllByText("OrderBreakdown.bookingEnd")
				).toHaveLength(expectedLength);
				...
				expect(
					screen.queryAllByText("OrderBreakdown.baseUnitDay")
				).toHaveLength(expectedLength);
				expect(
					screen.queryAllByText("OrderBreakdown.subTotal")
				).toHaveLength(expectedLength);
				...
				expect(screen.queryAllByText("$9.00")).toHaveLength(
					expectedTotal
				);
	...
	describe('default-booking process with "hour" (Sale)', () => {
		...
		test.each(bookings)(
			...
			async ({ tr, tx, isReversal, isReceived }) => {
				...
				expect(screen.queryAllByText("Tue 10:00 AM")).toHaveLength(
					expectedLength
				);
				expect(screen.queryAllByText("Tue 1:00 PM")).toHaveLength(
					expectedLength
				);
				expect(screen.queryAllByText("Jun 14")).toHaveLength(
					expectedLength * 2
				...
				expect(
					screen.queryAllByText("OrderBreakdown.baseUnitHour")
				).toHaveLength(expectedLength);
				expect(
					screen.queryAllByText("OrderBreakdown.subTotal")
				).toHaveLength(expectedLength);
	...
	describe("Transaction process with OrderPanel", () => {
		...
		it("Inquiry with OrderPanel - hour unit", async () => {
			...
			const { getByText, getAllByText } = render(
				<TransactionPageComponent {...props} />,
				{
					config,
					routeConfiguration,
				}
			...
			await waitFor(() => {
				...
				expect(
					getAllByText("OrderPanel.priceInMobileCTA")
				).toHaveLength(1);
				expect(getAllByText("OrderPanel.perUnit")).toHaveLength(1);

				// BookingTimeForm inputs
				expect(
					getByText("BookingTimeForm.bookingStartTitle")
				).toBeInTheDocument();
				expect(
					getByText("FieldDateAndTimeInput.startTime")
				).toBeInTheDocument();
				expect(
					getByText("FieldDateAndTimeInput.endTime")
				).toBeInTheDocument();
				expect(
					getByText("BookingTimeForm.requestToBook")
				).toBeInTheDocument();
			});
		});
		...
		it("Inquiry with restricted view rights should not show OrderPanel - hour unit", async () => {
			...
			currentUser.effectivePermissionSet.attributes.read =
				"permissions/deny";
			...
			await waitFor(() => {
				...
				expect(
					screen.queryByText("OrderPanel.priceInMobileCTA")
				).toBeNull();
				expect(screen.queryByText("OrderPanel.perUnit")).toBeNull();

				// Don't show BookingTimeForm inputs
				expect(
					screen.queryByText("BookingTimeForm.bookingStartTitle")
				).toBeNull();
				expect(
					screen.queryByText("FieldDateAndTimeInput.startTime")
				).toBeNull();
				expect(
					screen.queryByText("FieldDateAndTimeInput.endTime")
				).toBeNull();
				expect(
					screen.queryByText("BookingTimeForm.requestToBook")
				).toBeNull();
			});
		});
		...
		it("Inquiry with OrderPanel - day unit", async () => {
			...
			await waitFor(() => {
				...
				// BookingTimeForm inputs
				expect(
					getByText("BookingDatesForm.bookingStartTitle")
				).toBeInTheDocument();
				expect(
					getByText("BookingDatesForm.bookingEndTitle")
				).toBeInTheDocument();
				expect(
					getByText("BookingDatesForm.requestToBook")
				).toBeInTheDocument();
			});
		});
		...
		it("Inquiry with restricted view rights should not show OrderPanel - day unit", async () => {
			...
			await waitFor(() => {
				...
				expect(
					screen.queryByText("BookingDatesForm.bookingStartTitle")
				).toBeNull();
				expect(
					screen.queryByText("BookingDatesForm.bookingEndTitle")
				).toBeNull();
				expect(
					screen.queryByText("BookingDatesForm.requestToBook")
				).toBeNull();
			});
		});
		...
		it("Inquiry with OrderPanel - item unit", async () => {
			...
			await waitFor(() => {
				...
				// Order purchase CTA
				expect(
					getByText("OrderPanel.ctaButtonMessagePurchase")
				).toBeInTheDocument();
			});
		});
		...
		it("Inquiry with restricted view rights should not show OrderPanel - item unit", async () => {
			...
			await waitFor(() => {
				...
				expect(
					screen.queryByText("OrderPanel.ctaButtonMessagePurchase")
				).toBeNull();
			});
		});
	});
