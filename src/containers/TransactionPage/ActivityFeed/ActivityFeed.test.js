...
describe("ActivityFeed", () => {
	it("verify that messages and relevant transition are shown", () => {
		...
		const props = {
			messages: [
				createMessage(
					...
					{
						content: "message 1",
						createdAt: new Date(Date.UTC(2023, 10, 9, 8, 12)),
					},
					{ sender: customer }
				),
				createMessage(
					...
					{
						content: "message 2",
						createdAt: new Date(Date.UTC(2023, 10, 10, 8, 12)),
					},
					{ sender: provider }
				),
			],
		...
		expect(
			firstLI.getByText(
				"TransactionPage.ActivityFeed.default-purchase.purchased"
			)
		).toBeInTheDocument();
	...
	it("hide message if the sender is banned", () => {
		...
		expect(
			firstMsg.getByText("TransactionPage.messageSenderBanned")
		).toBeInTheDocument();
		expect(firstMsg.getByText("2023-11-09")).toBeInTheDocument();
