	...
	<FinalForm
		...
		render={fieldRenderProps => {
			...
			const classes = classNames(
				rootClassName || css.formRoot,
				className
			);
			const submitInProgress = counterOfferInProgress;
			const submitDisabled = invalid || disabled || submitInProgress;
			const currentOfferFormatted = currentOffer
				? formatMoney(intl, currentOffer)
				: "$10.00";
				...
				<Form className={classes} onSubmit={handleSubmit}>
					<FieldCurrencyInput
						...
						label={intl.formatMessage({
							id: "MakeCounterOfferForm.offerLabel",
						})}
						...
						validate={required(
							intl.formatMessage({
								id: "MakeCounterOfferForm.offerRequired",
							})
						)}
					/>
					...
					>
						{intl.formatMessage({
							id: "MakeCounterOfferForm.submit",
						})}
					</Button>
				</Form>
			);
	...
	return (
		<>
			...
			<p className={css.modalMessage}>
				<FormattedMessage
					id="MakeCounterOfferModal.description"
