...
const hasReachedMaxDurationSinceTransition = (
	condition,
	transitions,
	timeZone
) => {
	const sinceTransition = transitions.find(
		t => t.transition === condition.sinceTransition
	);
	if (sinceTransition) {
		const enteredAt = getStartOf(
			sinceTransition.createdAt,
			"day",
			timeZone
		);
		const expiresAt = getStartOf(
			enteredAt,
			"day",
			timeZone,
			condition.days,
...
const checkCondition = (condition, transitions, timeZone) => {
	if (condition.type === "durationSinceTransition") {
		return hasReachedMaxDurationSinceTransition(
			condition,
			transitions,
			timeZone
		);
	...
	return buttonProps.conditions.reduce(
		(acc, c) => {
			if (
				!acc.disabled &&
				c.action === "disable" &&
				checkCondition(c, transitions, timeZone)
			) {
				// Use disabledReason.translationKey if present
				const translationKey = c.disabledReason?.translationKey;
				const fallbackReason =
					"You cannot perform this action right now."; // This should not be shown ever.

				if (!translationKey) {
					console.warn(
						`Translation key not found for condition: ${c.type}`
					);
				}
		...
		primaryButtonProps && hasValidData
			? [
					...
					>
						...
						{primaryDisabled && (
							<div className={css.finePrint}>{primaryReason}</div>
						)}
					</div>,
			  ]
			: [];
