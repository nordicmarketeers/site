...
const FilterComponent = props => {
	...
	switch (schemaType) {
		case "category": {
			...
			const queryParamNames = nestedParams?.map(p =>
				constructQueryParamName(p, scope)
			);
			const label = intl.formatMessage({
				id: "FilterComponent.categoryLabel",
			});
		...
		case "listingType": {
			...
			const label = intl.formatMessage({
				id: "FilterComponent.listingTypeLabel",
			});
		...
		case "keywords":
			const label = intl.formatMessage({
				id: "FilterComponent.keywordsLabel",
			});
