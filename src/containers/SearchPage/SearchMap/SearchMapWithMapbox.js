...
const priceLabelsInLocations = (
	...
	const priceLabels = listingArraysInLocations.reverse().map(listingArr => {
		...
		if (listingArr.length === 1) {
			...
			if (
				infoCardOpen != null &&
				infoCardOpenIds.includes(listing.id.uuid)
			) {
				return null;
			}
...
class SearchMapWithMapbox extends Component {
	constructor(props) {
		...
		this.handleDoubleClickOnInfoCard = this.handleDoubleClickOnInfoCard.bind(
			this
		);
	...
	componentDidUpdate(prevProps) {
		...
		if (this.map) {
			...
			if (
				!isEqual(this.props.bounds, currentBounds) &&
				!this.viewportBounds
			) {
		...
		if (!this.map && this.state.mapContainer) {
			...
			prevProps.mapComponentRefreshToken !==
	...
	render() {
		...
		if (this.map) {
			...
			const createMarker = (data, markerContainer) =>
				new window.mapboxgl.Marker(markerContainer, {
					anchor: "bottom",
				})
			...
			this.currentMarkers = labels
				...
				.map(m => {
					const existingMarkerId = this.currentMarkers.findIndex(
						marker =>
							m.markerId === marker.markerId && marker.marker
					);
			...
			if (infoCardOpen) {
				...
				this.currentInfoCard = {
					...
					marker: infoCard
						? createMarker(infoCard, infoCardContainer)
						: null,
				};
			} else {
