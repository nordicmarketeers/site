...
export class SearchPageComponent extends Component {
	...
	onMapMoveEnd(viewportBoundsChanged, data) {
		...
		if (viewportBoundsChanged && isSearchPage) {
			...
			const { listingFields: listingFieldsConfig } =
				config?.listing || {};
			...
			const { address, bounds, mapSearch, page, ...rest } = parse(
				location.search,
				{
					latlng: ["origin"],
					latlngBounds: ["bounds"],
	...
	render() {
		...
		const builtInFilters = isKeywordSearch
			? defaultFiltersConfig.filter(
					f =>
						!["keywords", "categoryLevel", "listingType"].includes(
							f.key
						)
			  )
			: defaultFiltersConfig.filter(
			...
			return sortConfig.active ? (
				<SortBy
					...
					hasConflictingFilters={
						!!(sortConfig.conflictingFilters?.length > 0)
					}
				...
				<div id="main-content" className={css.container} role="main">
					<div className={css.searchResultContainer}>
						...
						>
							{availableFilters.map(filterConfig => {
								const key = `SearchFiltersMobile.${filterConfig.scope ||
									"built-in"}.${filterConfig.key}`;
						...
						>
							...
							>
								{availablePrimaryFilters.map(filterConfig => {
									const key = `SearchFiltersPrimary.${filterConfig.scope ||
										"built-in"}.${filterConfig.key}`;
						...
						{isSecondaryFiltersOpen ? (
							<div className={classNames(css.searchFiltersPanel)}>
								...
								>
									{customSecondaryFilters.map(
										filterConfig => {
											const key = `SearchFiltersSecondary.${filterConfig.scope ||
												"built-in"}.${
												filterConfig.key
											}`;
							...
							>
								...
								<SearchResultsPanel
									...
									listings={listings}
									pagination={
										listingsAreLoaded ? pagination : null
									}
					...
					>
						<div
							...
							data-testid="searchMapContainer"
						>
							{shouldShowSearchMap ? (
								<SearchMap
...
const EnhancedSearchPage = props => {
	...
	const hasNoViewingRightsError = isErrorNoViewingPermission(
		searchListingsError
	);
