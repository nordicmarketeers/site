...
const SortBy = props => {
	...
	const hasKeyworsFilter = config.search.defaultFilters.find(
		df => df.key === relevanceFilter
	);
	...
	const options = config.search.sortConfig.options.reduce(
		(selected, option) => {
			const isRelevance = option.key === relevanceKey;
			const isConflictingFilterSetAndActive =
			...
			const translationKeyLongMaybe = option?.labelTranslationKeyLong
				? {
						longLabel: intl.formatMessage({
							id: option?.labelTranslationKeyLong,
						}),
				  }
				: {};
			...
			return isRelevance && !isKeywordsFilterEnabled
				...
				: [
						...selected,
						{
							...
							disabled:
								(isRelevance &&
									(!isRelevanceOptionActive ||
										isConflictingFilterSetAndActive)) ||
	...
	const relevanceValue =
		isRelevanceSortActive && selectedFilters[relevanceFilter]?.length > 0
			? relevanceKey
			: null;
	const initialValue =
		hasConflictingFilters && !isConflictingFilterActive
