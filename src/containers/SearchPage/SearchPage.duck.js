...
const searchListingsPayloadCreator = ({ searchParams, config }, thunkAPI) => {
	...
	const prepareAPIParams = (params, paramHandlers) => {
		const pickedKeys = Object.entries(params).reduce((picked, [k, v]) => {
			...
			const currentParam =
				Object.keys(preparedParams).length > 0
					? preparedParams
					: { [k]: v };
	...
	const datesSearchParams = datesParam => {
		...
		const minDuration = isEntireRangeAvailable
			? dayCount * day - hour
			: hour;
	...
	return sdk.listings
		...
		.catch(e => {
			...
			if (
				!(isErrorUserPendingApproval(error) || isForbiddenError(error))
			) {
