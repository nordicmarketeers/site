...
const parse = priceRange => {
	...
	return !!priceRange && hasValue(minPrice) && hasValue(maxPrice)
		? { minPrice, maxPrice }
		: null;
};
...
const PriceFilter = props => {
	...
	const formatValidRangeValues = rangeValues => {
		...
		return hasValue(minValue) && hasValue(maxValue)
			? {
					minValue: formatCurrencyMajorUnit(
						intl,
						marketplaceCurrency,
						minValue
					),
					maxValue: formatCurrencyMajorUnit(
						intl,
						marketplaceCurrency,
						maxValue
					),
	...
	const hasInitialValues =
		initialValues && hasValue(minPrice) && hasValue(maxPrice);
	...
	const formattedRangeForAriaLabel = hasInitialValues
		? `${formatCurrencyMajorUnit(
				...
				marketplaceCurrency,
				minPrice
		  )} - ${formatCurrencyMajorUnit(intl, marketplaceCurrency, maxPrice)}`
		: "";
	const labelWithRange = hasInitialValues
		? intl.formatMessage(
				{ id: "PriceFilter.labelSelectedButton" },
				{
					minPrice: formatCurrencyMajorUnit(
						intl,
						marketplaceCurrency,
						minPrice
					),
					maxPrice: formatCurrencyMajorUnit(
						intl,
						marketplaceCurrency,
						maxPrice
					),
	...
	const getLabelForRangeInput = (priceInMajorUnit, handleName) => {
		const formattedPrice = formatCurrencyMajorUnit(
			intl,
			marketplaceCurrency,
			priceInMajorUnit
		);
