...
const GroupOfFieldCheckboxes = props => {
	...
	return (
		<fieldset className={className}>
			{legend ? (
				<legend className={css.accessibilityLegend}>{legend}</legend>
			) : null}
...
const format = (selectedOptions, queryParamName, schemaType, searchMode) => {
	...
	const mode =
		schemaType === SCHEMA_TYPE_MULTI_ENUM && searchMode
			? `${searchMode}:`
			: "";
	const value = hasOptionsSelected
		? `${mode}${selectedOptions.join(",")}`
		: null;
