	...
	positionStyleForContent() {
		if (this.filter && this.filterContent) {
			...
			const contentPlacementOffset =
				this.props.contentPlacementOffset || 0;
									...
									this.toggleIsOpen({
										callback: () => {
											delayedMoveFocus(
												formId,
												!isOpen,
												moveFocusToFirstFocusableElement
											);
										},
									});
								}
						...
						ArrowDown: {
							...
							callback: (...args) => {
								...
								if (
									...
									this.filterContent.contains(event.target) &&
									!isInput(event.target)
								) {
									event.preventDefault();
									event.stopPropagation();
									moveFocusToNextFocusableElement(
										formId,
										"next"
									);
								}
									...
									this.toggleIsOpen({
										callback: () => {
											delayedMoveFocus(
												formId,
												true,
												moveFocusToLastFocusableElement
											);
										},
										enforcedState: true,
									});
