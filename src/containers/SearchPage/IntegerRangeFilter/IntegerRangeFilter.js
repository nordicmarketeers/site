...
const convertQueryParamToObject = valueRange => {
	...
	return !!valueRange && minValue != null && maxValue != null
		? { minValue, maxValue }
		: null;
};
...
const getValidRangeValues = (queryParamNames, rangeParams, min, max) => {
	...
	const hasValidMaxValue =
		hasValue(maxValue) && maxValue >= minValue && maxValue <= max;
...
const formatToQueryParam = (range, queryParamName) => {
	...
	// We explicitly compare `minValue` to null as comparing to '0' returns false but '0' is a valid range value.
	const value =
		minValue != null && maxValue != null ? `${minValue},${maxValue}` : null;
...
const IntegerRangeFilter = props => {
	...
	const validRangeValues = getValidRangeValues(
		queryParamNames,
		initialValues,
		min,
		max
	...
	const labelSelectionForPlain = hasInitialValues ? (
		<FormattedMessage
			id="IntegerRangeFilter.labelSelectedPlain"
			values={formattedRangeValues}
		/>
	) : null;
