...
const getValidHandles = (currentValues, inputValues, minLimit, maxLimit) => {
	...
	if (isMinUpdated && isMaxUpdated) {
		...
		const inRange = !(
			isTooSmall(minValue, minLimit) || isTooBig(maxValue, maxLimit)
		);
...
const RangeInput = props => {
	...
	const currentValues = resolveMinMaxValues(
		initialValues[name],
		defaultMaxValue,
		defaultMinValue
	);
	...
	const isMinInvalid = !isValidMin(
		fieldValues.minValue,
		defaultMinValue,
		fieldValues.maxValue
	);
	const isMaxInvalid = !isValidMax(
		fieldValues.maxValue,
		defaultMaxValue,
		fieldValues.minValue
	);
		...
		<div className={classes}>
			<div
				className={classNames(css.contentWrapper, {
					[css.contentWrapperSidebar]: isInSideBar,
				})}
			>
				<div className={css.inputsWrapper}>
					{!isInSideBar ? (
						<span className={css.labelPopup}>
							{intl.formatMessage({
								id: "IntegerRangeFilter.rangeInputsLabel",
							})}
						</span>
					) : null}
					<input
						...
						aria-valuetext={labelForRangeInput(
							fieldValues.minValue,
							"min"
						)}
						aria-valuemin={defaultMinValue}
						aria-valuemax={defaultMaxValue}
						aria-label={labelForRangeInput(
							fieldValues.minValue,
							"min"
						)}
					></input>
