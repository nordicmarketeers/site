...
const listingTypeOptions = listingTypes.map(lt => ({
	option: lt.id,
	label: labelize(lt.id),
}));
...
describe("SearchPage", () => {
	...
	it("Check that filterColumn and filters exist in grid variant", async () => {
		...
		await waitFor(() => {
			...
			expect(
				screen.queryByTestId("searchMapContainer")
			...
			// Has Category filter
			expect(
				getByText("FilterComponent.categoryLabel")
			).toBeInTheDocument();
			...
			expect(
				getByText("FilterComponent.listingTypeLabel")
			).toBeInTheDocument();
	...
	it("Check that map and filters exist in map variant", async () => {
		...
		await waitFor(() => {
			...
			expect(
				screen.queryByTestId("filterColumnAside")
			).not.toBeInTheDocument();
			// Has search map container
			expect(
				screen.getByTestId("searchMapContainer")
			).toBeInTheDocument();
			...
			// Has Category filter
			expect(
				getByLabelText("Filter: FilterComponent.categoryLabel")
			).toBeInTheDocument();
			...
			expect(
				queryByText("Rent bicycles nightly")
			).not.toBeInTheDocument();
			...
			expect(
				getByText("SearchFiltersPrimary.moreFiltersButton")
			).toBeInTheDocument();
		...
		await waitFor(() => {
			userEvent.click(
				getByRole("button", {
					name: "Filter: FilterComponent.categoryLabel",
				})
			);
		});
		expect(getByText("Dogs")).toBeInTheDocument();
	...
	it("Check that Boat filters is revealed in grid variant", async () => {
		...
		await waitFor(() => {
			userEvent.click(
				getByRole("button", { name: "Choose Sell bicycles." })
			);
		});
	...
	it("Check that Listing type filter is not revealed when using a listing type path param", async () => {
		...
		const props = {
			...commonProps,
			params: { listingType: "sell-bicycles" },
		};
		const searchRouteConfig = routeConfiguration.find(
		...
		await waitFor(() => {
			...
			expect(
				queryByText("FilterComponent.listingTypeLabel")
			).not.toBeInTheDocument();
			expect(queryByText("Rent bicycles daily")).not.toBeInTheDocument();
			expect(
				queryByText("Rent bicycles nightly")
...
describe("Duck", () => {
	...
	it("loadData() for full viewing rights user loads listings", () => {
		...
		return loadData(null, null, config)(dispatch, getState, sdk).then(
			data => {
				...
				expect(actions[0].type).toBe(
					"SearchPage/searchListings/pending"
				);
				...
				expect(actions[2].type).toBe(
					"SearchPage/searchListings/fulfilled"
				);
