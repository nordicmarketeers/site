...
export const showUserThunk = createAsyncThunk(
	"ProfilePage/showUser",
	showUserPayloadCreator
);
	...
	return listingsPromise
		.then(response => {
			...
			const listingRefs = listings
				.filter(l => l =>
					!l.attributes.deleted && l.attributes.state === "published"
				)
				.map(({ id, type }) => ({ id, type }));
...
//////////////////////////
const queryUserReviewsPayloadCreator = (
	{ userId },
		...
		builder
			...
			.addCase(queryUserListingsThunk.pending, (state, action) => {
				...
				state.userListingRefs =
					userId === state.userId ? state.userListingRefs : [];
