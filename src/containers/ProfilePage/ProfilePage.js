...
export const DesktopReviews = props => {
		...
		? [
				{
					text: (
						<Heading
							as="h3"
							rootClassName={css.desktopReviewsTitle}
						>
							<FormattedMessage
								id="ProfilePage.reviewsFromMyCustomersTitle"
								values={{ count: reviewsOfProvider.length }}
	...
	const desktopReviewTabs = [
		...providerReviewsMaybe,
		...customerReviewsMaybe,
	];
			...
			<div className={css.desktopReviewsWrapper}>
				<ButtonTabNavHorizontal
					...
					ariaLabel={intl.formatMessage({
						id: "ProfilePage.screenreader.reviewsNav",
					})}
				/>
...
export const MainContent = props => {
	...
	const isMobileLayout =
		mounted && hasMatchMedia
			? window.matchMedia(`(max-width: ${MAX_MOBILE_SCREEN_WIDTH}px)`)
					?.matches
			: true;
...
export const ProfilePageComponent = props => {
	...
	const isPreview =
		isVariant &&
		pathParams.variant === PROFILE_PAGE_PENDING_APPROVAL_VARIANT;
	...
	const hasNoViewingRightsOnPrivateMarketplace =
		isPrivateMarketplace && hasNoViewingRightsUser;
	...
	if (!isDataLoaded) {
		...
		!isPreview &&
		(isUnauthorizedOnPrivateMarketplace || hasUserPendingApprovalError)
	) {
		return (
			<NamedRedirect
				name="NoAccessPage"
...
const mapStateToProps = state => {
	...
	const useCurrentUser =
		...
		!(
			...
			hasPermissionToViewData(currentUser)
		);
