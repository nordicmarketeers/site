...
describe("ProfilePage", () => {
	...
	it("Check that user name and bio is shown correctly", async () => {
		...
		expect(
			screen.getByText("ProfilePage.desktopHeading")
		).toBeInTheDocument();
	...
	it("Check that custom user information is shown correctly", async () => {
		...
		expect(
			getByRole("heading", { name: "ProfilePage.detailsTitle" })
		).toBeInTheDocument();
		expect(
			getByRole("heading", { name: "Dietary preferences" })
		).toBeInTheDocument();
		expect(
			getByRole("heading", { name: "Description of your kitchen" })
		).toBeInTheDocument();
		...
		expect(
			screen.getByText("How many cookbooks do you have")
		).toBeInTheDocument();
		expect(screen.getByText("10")).toBeInTheDocument();
		expect(
			screen.getByText("This is a kitchen description!")
		).toBeInTheDocument();
	...
	it("Check that listing information is shown correctly", async () => {
		...
		expect(
			screen.getByText("ProfilePage.listingsTitle")
		).toBeInTheDocument();
	...
	it("Check that review information is shown correctly", async () => {
		...
		expect(
			getByRole("heading", {
				name: "ProfilePage.reviewsFromMyCustomersTitle",
			})
		).toBeInTheDocument();
...
describe("Duck", () => {
	...
	it("loadData() for full viewing rights user loads someone else's profile", () => {
		...
		return loadData({ id: userId }, null, config)(
			dispatch,
			getState,
			sdk
		).then(data => {
			...
			const fulfilledActions = relevantActions.filter(action =>
			...
			expect(
				relevantActions.some(
					action => action.type === "ProfilePage/showUser/pending"
				)
			).toBe(true);
			expect(
				relevantActions.some(
					action =>
						action.type === "ProfilePage/queryUserListings/pending"
				)
			).toBe(true);
			expect(
				relevantActions.some(
					action =>
						action.type === "ProfilePage/queryUserReviews/pending"
				)
			).toBe(true);
			expect(
				relevantActions.some(
					action => action.type === "auth/authInfo/pending"
				)
			).toBe(true);
			expect(
				relevantActions.some(
					action => action.type === "ProfilePage/showUser/fulfilled"
				)
			).toBe(true);
			expect(
				relevantActions.some(
					action =>
						action.type ===
						"ProfilePage/queryUserListings/fulfilled"
				)
			).toBe(true);
			expect(
				relevantActions.some(
					action =>
						action.type === "ProfilePage/queryUserReviews/fulfilled"
				)
			).toBe(true);
			expect(
				relevantActions.some(
					action => action.type === "auth/authInfo/fulfilled"
				)
			).toBe(true);
		});
	});
	...
	it("loadData() for restricted viewing rights user does not load someone else's profile", () => {
		...
		// loadData() function is called. If you make customizations to the loadData() logic,
		// update the dispatched actions list in this test accordingly!
		return loadData({ id: "otherUserId" }, null, config)(
			dispatch,
			getState,
			sdk
		).then(data => {
			...
			const rejectedActions = relevantActions.filter(action =>
			...
			expect(
				relevantActions.some(
					action => action.type === "ProfilePage/showUser/rejected"
				)
			).toBe(true);
			expect(
				relevantActions.some(
					action =>
						action.type === "ProfilePage/queryUserListings/rejected"
				)
			).toBe(true);
			expect(
				relevantActions.some(
					action =>
						action.type === "ProfilePage/queryUserReviews/rejected"
				)
			).toBe(true);
			expect(
				relevantActions.some(
	...
	it("loadData() for restricted viewing rights user loads their own profile", () => {
		...
		return loadData({ id: userId }, null, config)(
			...
			expect(
				relevantActions.some(
					action => action.type === "ProfilePage/setUserId"
				)
			).toBe(true);
