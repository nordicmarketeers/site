...
describe("RequestQuotePage", () => {
	...
	it("Check that listing information is shown correctly", async () => {
		...
		expect(
			screen.getByLabelText(/RequestQuotePage\.defaultMessageLabel/)
		).toBeInTheDocument();

		// Check that the submit button is present
		expect(
			screen.getByRole("button", {
				name: /RequestQuotePage\.submitButtonText/,
			})
		).toBeInTheDocument();
			...
			return loadData({ id: listingId }, null, config)(
				...
				expect(
					relevantActions.some(
						action =>
							action.type ===
							"RequestQuotePage/showListing/pending"
					)
				).toBe(true);
				expect(
					relevantActions.some(
				...
				expect(
					relevantActions.some(
						action =>
							action.type ===
							"RequestQuotePage/showListing/fulfilled"
					)
				).toBe(true);
			});
		});
			...
			return loadData({ id: listingId }, null, privateConfig)(
				...
				expect(
					relevantActions.some(
						action =>
							action.type ===
							"RequestQuotePage/showTransaction/pending"
					)
				).toBe(false);
			});
		});
