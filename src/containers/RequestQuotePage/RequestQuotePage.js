...
const getProcessName = listing => {
	const processName = listing?.id
		? listing?.attributes?.publicData?.transactionProcessAlias?.split(
				"/"
		  )[0]
		: null;
...
const getTransactionTypeData = (listingType, unitTypeInPublicData, config) => {
	...
	return unitTypeInPublicData
		? { unitType: unitTypeInPublicData, ...rest }
		: {};
};
...
const handleSubmit = (submitting, setSubmitting, props) => values => {
	...
	onRequestQuote(requestQuoteParams, transactionProcessAlias)
		.then(transaction => {
			...
			const txDetailsPath = pathByRouteName(
				transactionPage,
				routeConfiguration,
				{
				...
				<main className={css.orderFormContainer}>
					...
					<section className={css.paymentContainer}>
						<RequestQuoteForm
							intl={intl}
							config={config}
							authorDisplayName={userDisplayNameAsString(
								listing?.author,
								""
							)}
							onSubmit={onSubmit}
...
const mapDispatchToProps = dispatch => ({
	onRequestQuote: (params, processAlias) =>
		dispatch(requestQuote(params, processAlias)),
});
