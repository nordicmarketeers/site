...
const Example = props => {
	...
	const exampleWrapperClassName = useDefaultWrapperStyles
		? css.defaultWrapperStyles
		: "";
	const desc = description ? (
		<p className={css.withMargin}>Description: {description}</p>
	) : null;
	return (
...
const Nav = props => {
	...
	const prefixGroups = filteredGroups.reduce((acc, g) => {
		...
		const prevUnprefixedGroups =
			acc && acc.unprefixed ? acc.unprefixed : [];
		return { ...acc, unprefixed: [...prevUnprefixedGroups, g] };
	}, {});

	const getGroupLinks = (prefixGroups, prefix) =>
		prefix && prefixGroups[prefix]
			? prefixGroups[prefix].map(g =>
					toGroupLink(g, g.slice(prefix.length + 1))
			  )
			: !prefix
...
const StyleguidePage = props => {
	...
	const prefixIndex = selectedGroup
		? selectedGroup.indexOf(PREFIX_SEPARATOR)
		: -1;
	const selectedGroupWithoutPrefix =
		prefixIndex > 0
			? selectedGroup.slice(prefixIndex + 1).trim()
			: selectedGroup;
	return (
