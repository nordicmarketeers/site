...
class ContactDetailsFormComponent extends Component {
	constructor(props) {
		...
		this.handleResendVerificationEmail = this.handleResendVerificationEmail.bind(
			this
		);
		...
		return (
			<FinalForm
				...
				render={fieldRenderProps => {
					...
					const emailTouched =
						this.submittedValues.email !== values.email;
					const emailTakenErrorText = isChangeEmailTakenError(
						saveEmailError
					)
						? intl.formatMessage({
					...
					const passwordTouched =
						this.submittedValues.currentPassword !==
					...
					const isGenericEmailError =
						saveEmailError &&
						!(emailTakenErrorText || passwordErrorText);
