...
const calculateContainerWidth = (containerRefTarget, parentWidth) => {
	...
	const siblingArray = containerRefTarget?.parentNode?.childNodes
		? Array.from(containerRefTarget.parentNode.childNodes).filter(
				n => n !== containerRefTarget
		  )
		: [];
	const siblingWidthsCombined = siblingArray.reduce(
		(acc, node) => acc + node.offsetWidth,
		0
	);
	...
	const availableContainerWidth =
		parentWidth - siblingWidthsCombined - padding;
			...
			observer.current = new ResizeObserver(entries => {
				...
				for (const entry of entries) {
					...
					const hasWidthOfTopbarDesktopChanged =
						!isBodyTheTarget &&
						containerRefParentWidth !== bodyOffsetWidth;

					if (isBodyTheTarget || hasWidthOfTopbarDesktopChanged) {
						const target = container;
						const availableContainerWidth = calculateContainerWidth(
							target,
							bodyOffsetWidth
						);

						// The groupedLinks variable contains { priorityLinks, menuLinks }
						const groupedLinks = groupMeasuredLinks(
							links,
