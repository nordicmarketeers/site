...
const SectionBuilder = ({ sections: originalSections = [], options = {} }) => {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      try {
        const cListings = await dispatch(fetchListingsByIds(consultantsListingIds));
        setconsultantListings(cListings);

        const jListings = await dispatch(fetchListingsByIds(jobsListingIds));
  ...
  if (sections[0]?.sectionName === 'Marketplace introduction') {
    sections.splice(
      2,
      0,
      <CustomConsultantsSection key="custom-consultants" consultantListings={consultantListings} />
    );
    sections.splice(4, 0, <CustomJobsSection key="custom-jobs" jobListings={jobListings} />);
  }
  ...
  const getUniqueSectionId = (sectionId, index) => {
    ...
    if (sectionIds.includes(candidate)) {
      ...
      while (sectionIds.includes(newCandidate)) i++, (newCandidate = `${candidate}${i}`);
