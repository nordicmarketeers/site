...
export const fetchListingsByIds = listingIds => async dispatch => {
  const listings = await Promise.all(listingIds.map(id => dispatch(fetchListingById(id))));
  ...
  const cleanListings = listings.flatMap(res => {
    ...
    const author = included.find(i => i.type === 'user' && i.id.uuid === authorId) || null;

    // Author's profile image
    const profileImageId = author?.relationships?.profileImage?.data?.id?.uuid;
    const profileImage =
      included.find(i => i.type === 'image' && i.id.uuid === profileImageId) || null;
