...
describe("Field helpers", () => {
	describe("hasContent(data)", () => {
		it('should return true if data has "content"', () => {
			...
			expect(
				hasContent({ content: "Hello world!", blaa: "blaa" })
			).toEqual(true);
		});
	...
	describe("exposeContentAsChildren(data)", () => {
		it('should return only "children" prop containing the string from passed-in "content"', () => {
			expect(
				exposeContentAsChildren({ content: "Hello world!" })
			).toEqual({
	...
	describe("exposeContentString(data)", () => {
		it('should return only "children" prop containing the string from passed-in "content"', () => {
			...
			expect(
				exposeContentString({ content: "Hello world!", blaa: "blaa" })
			).toEqual({
				content: "Hello world!",
			});
		});
	...
	describe("exposeLinkProps(data)", () => {
		...
		it("should return empty object if data is not valid", () => {
			expect(
				exposeLinkProps({ content: "Hello world!", blaa: "blaa" })
			).toEqual({});
		});
		it('should return href as "children" if label is not valid', () => {
			const href = "https://my.example.com/some/image.png";
			expect(exposeLinkProps({ href })).toEqual({
				children: href,
				href: href,
			});
			expect(exposeLinkProps({ content: 0, href })).toEqual({
				children: href,
				href: href,
			});
		});
	...
	describe("exposeImageProps(data)", () => {
		it('should return only "alt", "variants" and "link" props', () => {
			...
			expect(
				exposeImageProps({ alt: "Hello world!", image, blaa: "blaa" })
			).toEqual({
				alt: "Hello world!",
				image,
				link: null,
		...
		it("should return empty object if data is not valid", () => {
			...
			expect(
				exposeLinkProps({ alt: "Hello world!", blaa: "blaa" })
			).toEqual({});
	...
	describe("exposeCustomAppearanceProps(data)", () => {
		it('should return "backgroundColor" prop containing valid hexadecimal color code', () => {
			expect(
				exposeCustomAppearanceProps({ backgroundColor: "#FFAA00" })
			).toEqual({
				backgroundColor: "#FFAA00",
			});
			expect(
				exposeCustomAppearanceProps({ backgroundColor: "#FA0" })
			).toEqual({
				backgroundColor: "#FA0",
			});
			expect(
		...
		it('should return empty "backgroundColor" prop if invalid hexadecimal color code was detected', () => {
			expect(
				exposeCustomAppearanceProps({ backgroundColor: "#FFAA0000" })
			).toEqual({});
			expect(
				exposeCustomAppearanceProps({ backgroundColor: "FA0" })
			).toEqual({});
			expect(
				exposeCustomAppearanceProps({ backgroundColor: "000000" })
			).toEqual({});
			expect(
				exposeCustomAppearanceProps({ backgroundColor: "#XX0000" })
			).toEqual({});
			expect(
				exposeCustomAppearanceProps({ backgroundColor: "#FFAA0" })
			).toEqual({});
			expect(
				exposeCustomAppearanceProps({
			...
			expect(
				exposeCustomAppearanceProps({
					backgroundColor: "hwb(90 10% 10%)",
				})
			).toEqual({});
			expect(
				exposeCustomAppearanceProps({ backgroundColor: "tomato" })
			).toEqual({});
		});

		it('should return "textColor" prop containing valid value (light or dark)', () => {
			expect(exposeCustomAppearanceProps({ textColor: "light" })).toEqual(
				{ textColor: "light" }
			);
			expect(exposeCustomAppearanceProps({ textColor: "dark" })).toEqual({
				textColor: "dark",
			});
		});
		...
		it('should return "backgroundImage" prop containing valid imageAsset', () => {
			...
			expect(
				exposeCustomAppearanceProps({ backgroundImage, alt })
			).toEqual({
				backgroundImage,
				alt,
			});
	...
	describe("exposeYoutubeProps(data)", () => {
		it('should return "youtubeVideoId" prop ', () => {
			...
			expect(exposeYoutubeProps({ youtubeVideoId })).toEqual({
				youtubeVideoId,
			});
		});
		it('should return empty object if invalid "youtubeVideoId" was detected', () => {
			expect(
				exposeYoutubeProps({ youtubeVideoId: "9RQli?kX4vvw" })
			).toEqual({});
			expect(
				exposeYoutubeProps({ youtubeVideoId: "9RQli&kX4vvw" })
			).toEqual({});
