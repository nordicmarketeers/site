 ...
 */
export const exposeContentString = data =>
	hasContent(data) ? { content: data.content } : {};
...
export const exposeSocialMediaProps = data => {
	...
	const validPlatform = supportedPlatforms.includes(platform)
		? platform
		: null;
...
export const exposeImageProps = data => {
	...
	const cleanUrl = hasCorrectProps ? sanitizeUrl(href) : null;
	const linkData =
		cleanUrl && fieldType !== "none" ? { href: cleanUrl, fieldType } : null;

	const alternativeText = typeof alt === "string" ? alt : "🖼️";
	const sanitizedImage = getValidSanitizedImage(image);

	return sanitizedImage
		? { alt: alternativeText, image: sanitizedImage, link: linkData }
		: {};
};
...
export const exposeCustomAppearanceProps = data => {
	...
	const backgroundImageMaybe = sanitizedImage
		? { backgroundImage: sanitizedImage, alt }
		: {};
	...
	const { preset, color: overlayColor, opacity: overlayOpacity } =
		backgroundImageOverlay || {};
	...
	const backgroundImageOverlayMaybe = hasBackgroundOverlay
		? {
				backgroundImageOverlay: {
					...
					opacity:
						typeof overlayOpacity === "number" ? overlayOpacity : 1,
				},
		  }
		: {};
...
export const exposeYoutubeProps = data => {
	...
	const cleanYoutubeVideoId = hasYoutubeVideoId
		? encodeURIComponent(youtubeVideoId)
		: null;

	const hasAspectRatio =
		isString(aspectRatio) && aspectRatio.match(/^(\d+)\/(\d+)+$/);
