...
const MarkdownField = ({ content, components }) =>
...
const defaultFieldComponents = {
	...
	socialMediaLink: {
		...
		pickValidProps: exposeSocialMediaProps,
	},
	image: { component: FieldImage, pickValidProps: exposeImageProps },
	customAppearance: {
		component: CustomAppearance,
		pickValidProps: exposeCustomAppearanceProps,
	},
...
const hasEmptyTextContent = obj =>
	hasExactNumKeys(obj, 2) &&
	TEXT_CONTENT.includes(obj?.fieldType) &&
...
export const validProps = (data, options) => {
	...
	if (data && pickValidProps) {
		...
		const omitWarning =
			config?.omitInvalidPropsWarning &&
			config?.omitInvalidPropsWarning(data);
		...
		if (Object.keys(validProps).length === 0 && !omitWarning) {
			console.warn(
				`Invalid props detected. Data: ${JSON.stringify(data)}`
			);
		}
		return validProps;
	}
	...
	if (data && !config) {
		...
		console.warn(
			`Unknown field type (${
				data?.fieldType
			}) detected. Data: ${JSON.stringify(data)}`
		);
	} else if (data && !pickValidProps) {
		console.warn(
			`There's no validator (pickValidProps) for this field type (${data?.fieldType}).`
		);
	}
	return null;
};
...
export const hasDataInFields = (fields, fieldOptions) => {
	const hasData = fields.reduce((hasFoundValues, fieldData) => {
		...
		const hasDataInCurrent =
			validPropsFromData && Object.keys(validPropsFromData).length > 0;
