		...
		marketplaceData: {
			entities: {
				listing: {
					[listingId]: {
						...listing,
						relationships: { author: currentUser },
					},
				},
				transaction: {
...
describe("MakeOfferPage", () => {
	...
	it("Check that listing information is shown correctly", async () => {
		...
		expect(
			screen.getByLabelText(/MakeOfferPage\.quoteLabel/)
		).toBeInTheDocument();

		// Check that the message textarea is present
		expect(
			screen.getByLabelText(/MakeOfferPage\.defaultMessageLabel/)
		).toBeInTheDocument();

		// Check that the submit button is present
		expect(
			screen.getByRole("button", {
				name: /MakeOfferPage\.submitButtonText/,
			})
		).toBeInTheDocument();
...
describe("Duck", () => {
	...
	describe("loadData", () => {
		...
		it("loadData() for listing page loads listing data", () => {
			...
			const listing =
				initialState.marketplaceData.entities.listing[listingId];
			...
			return loadData({ id: listingId }, null, config)(
				dispatch,
				getState,
				sdk
			).then(data => {
				...
				expect(
					relevantActions.some(
						action =>
							action.type === "MakeOfferPage/showListing/pending"
					)
				).toBe(true);
				expect(
					relevantActions.some(
				...
				expect(
					relevantActions.some(
						action =>
							action.type ===
							"MakeOfferPage/showListing/fulfilled"
					)
				).toBe(true);
			});
		});
		...
		it("loadData() for transaction page loads transaction data", () => {
			...
			const { currentUser } = initialState.user;
			const transaction =
				initialState.marketplaceData.entities.transaction[
					transactionId
				];
			...
			return loadData(
				...
				config
			)(dispatch, getState, sdk).then(data => {
				const relevantActions = actions.filter(
					action => !action.type.startsWith("user/fetchCurrentUser/")
				);
				expect(relevantActions[0]).toEqual(setInitialState());
				expect(
					relevantActions.some(
						action =>
							action.type ===
							"MakeOfferPage/showTransaction/pending"
					)
				).toBe(true);
				expect(
					relevantActions.some(
				...
				expect(
					relevantActions.some(
						action =>
							action.type ===
							"MakeOfferPage/showTransaction/fulfilled"
					)
				).toBe(true);
			});
		});
		...
		it("loadData() for unauthorized user in private marketplace does not load data", () => {
			...
			return loadData({ id: listingId }, null, privateConfig)(
				dispatch,
				getState,
				sdk
			).then(data => {
