...
export const SocialLoginButtonsMaybe = props => {
	...
	const getDataForSSORoutes = () => {
		...
		const defaultReturn = pathByRouteName(
			"LandingPage",
			routeConfiguration
		);

		// Route for confirming user data before creating a new user
		const defaultConfirm = pathByRouteName(
			"ConfirmPage",
			routeConfiguration
		);
			...
			{showFacebookLogin ? (
				<div className={css.socialButtonWrapper}>
					<SocialLoginButton onClick={() => authWithFacebook()}>
						...
						<span id="facebook-authentication-msg">
							{facebookAuthenticationMessage}
						</span>
					</SocialLoginButton>
				</div>
			) : null}
			...
			{showGoogleLogin ? (
				<div className={css.socialButtonWrapper}>
					<SocialLoginButton onClick={() => authWithGoogle()}>
						...
						<span id="google-authentication-msg">
							{googleAuthenticationMessage}
						</span>
					</SocialLoginButton>
				</div>
			) : null}
			...
			>
				<div className={css.privacyWrapper} role="complementary">
					<PrivacyPolicyContent
						...
						error={tosFetchError}
						data={
							tosAssetsData?.[camelize(PRIVACY_POLICY_ASSET_NAME)]
								?.data
						}
					/>
				</div>
