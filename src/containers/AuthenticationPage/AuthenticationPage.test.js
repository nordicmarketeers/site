...
describe("AuthenticationPage", () => {
	...
	it("has just email and password inputs in login tab if social logins are not enabled", async () => {
		...
		expect(
			screen.queryByRole("button", {
				name: "AuthenticationPage.loginWithFacebook",
			})
		).not.toBeInTheDocument();
		expect(
			screen.queryByRole("button", {
				name: "AuthenticationPage.loginWithGoogle",
			})
		).not.toBeInTheDocument();
	});
	...
	it('changes the login form to sign up form by clicking "Sign up" ', async () => {
		...
		expect(
			screen.getByRole("button", { name: "LoginForm.logIn" })
		).toBeInTheDocument();
		...
		await act(async () => {
			...
			userEvent.click(
				screen.getByRole("link", {
					name: "AuthenticationPage.signupLinkText",
				})
			);
		});
		...
		waitFor(() =>
			expect(
				screen.findByRole("button", { name: "SignupForm.signUp" })
			).toBeInTheDocument()
		);
	});
});
...
describe("AuthenticationPage with SSO", () => {
	...
	it("has social login buttons on login tab when the env variables are in place", () => {
		...
		// for social logins are as we expect them to be
		process.env = Object.assign(process.env, {
			REACT_APP_FACEBOOK_APP_ID: "test-fb",
		});
		process.env = Object.assign(process.env, {
			REACT_APP_GOOGLE_CLIENT_ID: "test-google",
		});
