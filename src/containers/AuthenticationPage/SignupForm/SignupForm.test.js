...
describe("SignupForm", () => {
	...
	it("enables Sign up button when required fields are filled", async () => {
		...
		expect(
			screen.getByRole("button", { name: "SignupForm.signUp" })
		).toBeDisabled();
		...
		userEvent.type(
			screen.getByRole("textbox", { name: "SignupForm.firstNameLabel" }),
			"Joe"
		);
		userEvent.type(
			screen.getByRole("textbox", { name: "SignupForm.lastNameLabel" }),
			"Dunphy"
		);
		userEvent.type(
			screen.getByLabelText("SignupForm.passwordLabel"),
			"secret-password"
		);
