...
export const ManageListingsPageComponent = props => {
	...
	useEffect(() => {
		if (isErrorNoPermissionToPostListings(openingListingError?.error)) {
			const noAccessPagePath = pathByRouteName(
				"NoAccessPage",
				routeConfiguration,
				{
	...
	const discardingErrorListingId =
		!!discardingDraftError && discardingDraft.listingId;
				...
				<div className={css.listingPanel}>
					...
					<div className={css.listingCards}>
						{listings.map(l => (
							<ManageListingCard
								...
								isMenuOpen={
									!!listingMenuOpen &&
									listingMenuOpen.id.uuid === l.id.uuid
								}
								...
								hasOpeningError={
									openingErrorListingId.uuid === l.id.uuid
								}
								hasClosingError={
									closingErrorListingId.uuid === l.id.uuid
								}
								hasDiscardingError={
									discardingErrorListingId.uuid === l.id.uuid
								}
					...
					{onManageDisableScrolling && discardDraftModalOpen ? (
						<DiscardDraftModal
							...
							focusElementId={
								discardDraftModalId
									? `discardButton_${discardDraftModalId.uuid}`
									: null
							}
						/>
					) : null}
