...
export const ManageAccountPageComponent = props => {
	...
	const nonPublicUserFields = userFields.filter(uf =>
		["private", "protected"].includes(uf.scope)
	);
	...
	const hasUserTypeFields =
		...
		!!nonPublicUserFields.find(
			uf =>
				...
				(uf.userTypeConfig.limitToUserTypeIds &&
					uf.userTypeConfig.userTypeIds.includes(
					...
					{hasUserTypeFields ? (
						<>
							<PrivateDetailsForm
								...
								onSubmit={values =>
									handleFieldSubmit(values, userType)
								}
