...
const ErrorMessage = props => {
	...
	const unfinishedTransactionsError = isErrorUserHasUnfinishedTransactions(
		error
	);
	...
	const stripeDeletionFailedError = isStripeDeletionFailedNonZeroBalance(
		error
	);
		...
		<FinalForm
			...
			render={fieldRenderProps => {
				...
				const deleteAccountConfirmed =
					Array.isArray(confirmDeleteAccount) &&
					confirmDeleteAccount.length > 0;
				...
				const passwordErrorText = isChangeEmailWrongPassword(
					deleteAccountError
				)
				...
				const submitDisabled =
					invalid ||
					inProgress ||
					!(deleteAccountConfirmed && currentPassword);
					...
					>
						<FieldCheckbox
							...
							label={intl.formatMessage({
								id: "DeleteAccountForm.checkboxLabel",
							})}
						...
						<div
							...
							aria-hidden={!deleteAccountConfirmed}
						>
							...
							<p className={css.confirmChangesInfo}>
								<FormattedMessage
									id="DeleteAccountForm.deleteAccountInfo"
									values={{ email }}
								/>
								<br />
								<FormattedMessage
