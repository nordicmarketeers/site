...
describe("ProfileSettingsForm", () => {
	it("shows inputs and initial values for name", () => {
		...
		expect(
			screen.getByRole("heading", {
				name: "ProfileSettingsForm.yourName",
			})
		).toBeInTheDocument();
		expect(
	...
	it("shows inputs and initial values for bio", () => {
		...
		expect(
			screen.getByRole("heading", {
				name: "ProfileSettingsForm.bioHeading",
			})
		).toBeInTheDocument();
		expect(
			screen.getByText("ProfileSettingsForm.bioLabel")
	...
	it("shows a select input for enum fields", () => {
		...
		expect(screen.getByRole("option", { name: "e1l1" }).selected).toBe(
			true
		);
	});
	...
	it("shows a numeric input for long fields", () => {
		...
		const longInput = screen.getByRole("spinbutton", {
			name: "Long Field",
		});
	...
	it("shows a group checkbox input for multi-enum fields", () => {
		...
		expect(screen.getByRole("checkbox", { name: "ml1" }).checked).toBe(
			true
		);
		expect(screen.getByRole("checkbox", { name: "ml2" }).checked).toBe(
			true
		);
		expect(screen.getByRole("checkbox", { name: "ml3" }).checked).toBe(
			false
		);
	});
	...
	it("enables Save button when required fields are filled", () => {
		...
		expect(
			screen.getByRole("button", {
				name: "ProfileSettingsForm.saveChanges",
			})
		).toBeDisabled();
		...
		const textInput2 = screen.getByRole("textbox", {
			name: "Text Field 2",
		});
