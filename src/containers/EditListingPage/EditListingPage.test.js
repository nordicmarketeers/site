...
describe("EditListingPage", () => {
	...
	it("Purchase: new listing flow with categories", async () => {
		...
		await waitFor(() => {
			...
			// Tab: panel title
			expect(
				getByText("EditListingDetailsPanel.createListingTitle")
			).toBeInTheDocument();
		});
		...
		expect(
			queryAllByRole("option", {
				name: "EditListingDetailsForm.categoryPlaceholder",
			})[0].selected
		).toBe(false);
		expect(getByRole("option", { name: "Sneakers" }).selected).toBe(true);
		expect(
			queryAllByText("EditListingDetailsForm.categoryLabel")
		).toHaveLength(2);
		...
		await waitFor(() => {
			...
			userEvent.selectOptions(
				selectSubcategory,
				screen.getByRole("option", { name: "Adidas" })
			);
		});
		...
		await waitFor(() => {
			expect(
				getByRole("textbox", { name: "EditListingDetailsForm.title" })
			).toBeInTheDocument();
			//
			expect(
				getByRole("textbox", {
					name: "EditListingDetailsForm.description",
				})
			).toBeInTheDocument();
			...
			expect(
				getByRole("option", {
					name: "CustomExtendedDataField.placeholderSingleSelect",
				}).selected
			).toBe(true);
			//
			...
			expect(
				getByRole("button", {
					name: "EditListingWizard.default-purchase.new.saveDetails",
				})
			).toBeInTheDocument();
		});
	});
	...
	it("Purchase: edit existing listings that has no predefined categories", async () => {
		...
		await waitFor(() => {
			...
			// Tab: panel title
			expect(
				getByText("EditListingDetailsPanel.title")
			).toBeInTheDocument();
		});
	...
	it("Purchase: edit flow on details tab", async () => {
		...
		await waitFor(() => {
			...
			expect(
				getByRole("button", {
					name: "EditListingWizard.edit.saveDetails",
				})
			).toBeInTheDocument();
		});
	...
	it("Purchase: edit flow on pricing-and-stock tab", async () => {
		...
		await waitFor(() => {
			...
			// Tab: panel title
			expect(
				getByText("EditListingPricingAndStockPanel.title")
			).toBeInTheDocument();

			// Tab/form: price
			expect(
				getByRole("textbox", {
					name: "EditListingPricingAndStockForm.pricePerProduct",
				})
			).toHaveValue("$55.00");
			...
			expect(
				getByRole("spinbutton", {
					name: "EditListingPricingAndStockForm.stockLabel",
				})
			).toHaveValue(5);
	...
	it("Purchase: edit flow on delivery tab", async () => {
		...
		await waitFor(() => {
			...
			// Tab: panel title
			expect(
				getByText("EditListingDeliveryPanel.title")
			).toBeInTheDocument();

			expect(
				getByText("EditListingDeliveryForm.shippingLabel")
			).toBeInTheDocument();

			// Tab/form: pickup
			expect(
				getByRole("checkbox", {
					name: /EditListingDeliveryForm.pickupLabel/i,
				})
			).not.toBeChecked();
			expect(
				queryByPlaceholderText(
					"EditListingDeliveryForm.addressPlaceholder"
				)
			).toBeDisabled();
			expect(
				getByRole("textbox", {
					name: "EditListingDeliveryForm.building",
				})
			).toBeDisabled();
			...
			expect(
				getByRole("checkbox", {
					name: /EditListingDeliveryForm.shippingLabel/i,
				})
			).not.toBeChecked();
			expect(
				getByRole("textbox", {
					name: "EditListingDeliveryForm.shippingOneItemLabel",
				})
			).toBeDisabled();
			expect(
				getByRole("textbox", {
					name:
						"EditListingDeliveryForm.shippingAdditionalItemsLabel",
				})
			).toBeDisabled();

			expect(
				getByRole("button", {
					name: "EditListingWizard.edit.saveDelivery",
				})
			).toBeInTheDocument();
		});
		...
		expect(
			getByRole("textbox", {
				name: "EditListingDeliveryForm.shippingAdditionalItemsLabel",
			})
		).toBeEnabled();
	});
	...
	it("Purchase: edit flow on photos tab", async () => {
		...
		await waitFor(() => {
			...
			// Tab: panel title
			expect(
				getByText("EditListingPhotosPanel.title")
			).toBeInTheDocument();

			expect(
				getByText("EditListingPhotosForm.chooseImage")
			).toBeInTheDocument();
			expect(
				getByText("EditListingPhotosForm.imageTypes")
			).toBeInTheDocument();
			expect(
				getByText("EditListingPhotosForm.addImagesTip")
			).toBeInTheDocument();
			expect(
				getByText("EditListingWizard.edit.savePhotos")
			).toBeInTheDocument();
		});
	});
	...
	it("Purchase: edit flow with infinity on pricing-and-stock tab", async () => {
		...
		await waitFor(() => {
			...
			expect(
				getByRole("checkbox", {
					name: /EditListingPricingAndStockForm.updateToInfinite/i,
				})
			).not.toBeChecked();
	...
	it("Purchase: edit flow no pickup on delivery tab", async () => {
		const listingTypePurchase = listingTypesPurchase[0];
		const purchaseNoPickup = {
			...listingTypePurchase,
			defaultListingFields: { pickup: false },
		};
	...
	it("Booking (day): edit flow on location tab", async () => {
		...
		await waitFor(() => {
			...
			// Tab: panel title
			expect(
				getByText("EditListingLocationPanel.title")
			).toBeInTheDocument();
			...
			).toHaveValue("Main Street 123");

			// Tab/form: existing building
			expect(
				getByLabelText("EditListingLocationForm.building")
			).toHaveValue("A 1");

			expect(
				getByRole("button", {
					name: "EditListingWizard.edit.saveLocation",
				})
			).toBeInTheDocument();
		});
		...
		// Tab/form: existing building
		expect(getByLabelText("EditListingLocationForm.building")).toHaveValue(
			"B 2"
		);
	});
	...
	it("Booking (day): edit flow on pricing tab", async () => {
		...
		await waitFor(() => {
			...
			// Tab: panel title
			expect(
				getByText("EditListingPricingPanel.title")
			).toBeInTheDocument();

			// Tab/form: price
			expect(
				getByRole("textbox", {
					name: "EditListingPricingForm.pricePerProduct",
				})
			).toHaveValue("$10.00");

			expect(
				getByRole("button", {
					name: "EditListingWizard.edit.savePricing",
				})
			).toBeInTheDocument();
		});
		...
		// Tab/form: existing building
		expect(
			getByLabelText("EditListingPricingForm.pricePerProduct")
		).toHaveValue("$12.00");
	});
	...
	it("Booking (day): edit flow on availability tab", async () => {
		...
		await waitFor(() => {
			...
			// Tab: panel title
			expect(
				getByText("EditListingAvailabilityPanel.title")
			).toBeInTheDocument();

			// Tab/form: edit availability
			expect(
				getByRole("button", {
					name: /EditListingAvailabilityPanel.editAvailabilityPlan/i,
				})
			).toBeInTheDocument();
			...
			expect(
				getByRole("button", {
					name: "EditListingAvailabilityPanel.addException",
				})
			).toBeInTheDocument();
		});
	});
	...
	it("Booking (day): edit flow on availability tab (plan modal)", async () => {
		...
		expect(
			getByText("EditListingAvailabilityPlanForm.title")
		).toBeInTheDocument();
		// time zone picker
		expect(
			getByText("EditListingAvailabilityPlanForm.timezonePickerTitle")
		).toBeInTheDocument();
		expect(
			getByRole("option", { name: "Europe/Helsinki" })
		).toBeInTheDocument();
		// plan scheduler
		expect(
			getByText("EditListingAvailabilityPlanForm.hoursOfOperationTitle")
		).toBeInTheDocument();
		...
		expect(
			getByRole("button", {
				name: "EditListingAvailabilityPlanForm.saveSchedule",
			})
		).toBeInTheDocument();
		...
		await waitFor(async () => {
			await userEvent.click(
				getByRole("button", { name: /Modal.close/i })
			);
		});

		expect(
			queryAllByText("EditListingAvailabilityPlanForm.title")
		).toHaveLength(0);
	}, 10000);
	...
	it("Booking (day): edit flow on availability tab (exception modal)", async () => {
		...
		await waitFor(async () => {
			await userEvent.click(
				getByRole("button", {
					name: /EditListingAvailabilityPanel.addException/i,
				})
			);
		});
		expect(
			getByText("EditListingAvailabilityExceptionForm.title")
		).toBeInTheDocument();
		// mode: available, not-available
		expect(
			getByText("EditListingAvailabilityExceptionForm.available")
		).toBeInTheDocument();
		expect(
			getByText("EditListingAvailabilityExceptionForm.notAvailable")
		).toBeInTheDocument();
		...
		await waitFor(async () => {
			...
			expect(
				getByRole("button", {
					name: "EditListingAvailabilityExceptionForm.addException",
				})
			).toBeInTheDocument();
		});
	}, 10000);
	...
	it("Booking (day): edit flow on availability tab with seats", async () => {
		const config = getConfig(
			listingTypesBookingDayWithSeats,
			listingFieldsBooking
		);
		const routeConfiguration = getRouteConfiguration(config.layout);
		const listing = createOwnListing("listing-day", {
			title: "the listing",
	...
	it("Booking (day): edit flow on availability tab with seats (plan modal)", async () => {
		...
		expect(queryAllByLabelText("FieldSeatsInput.seatsLabel")).toHaveLength(
			7
		);
	...
	it("Booking (day): edit flow on availability tab with seats (exception modal)", async () => {
		...
		expect(
			queryByText("EditListingAvailabilityExceptionForm.available")
	...
	it("Booking (hour): edit flow on availability tab", async () => {
		...
		await waitFor(() => {
			...
			expect(
				cellMon.nextElementSibling.getElementsByClassName(
					"availabilityDot"
				)
			).toHaveLength(1);
			expect(
			...
			).toHaveClass("isAvailable");
			const sunday = getByText("Sunday");
			const cellSun = sunday.parentNode.parentNode;
			expect(
				cellSun.nextElementSibling.getElementsByClassName(
					"availabilityDot"
				)
			).toHaveLength(1);
		...
		const monDataContainerAfterUncheck = within(
			monday.parentNode.parentNode.nextElementSibling
		);
		expect(
	...
	it("Inquiry: edit flow no pricing on details tab", async () => {
		...
		const inquiryNoPricing = {
			...listingTypeInquiry,
			defaultListingFields: { price: false },
		};
	...
	it("Negotiation: edit flow with only details and style panels", async () => {
		...
		expect(
			getByText("EditListingWizard.tabLabelDetails")
		).toBeInTheDocument();

		// Style tab should be visible (style panel)
		expect(
			getByText("EditListingWizard.tabLabelStyle")
		).toBeInTheDocument();
		...
		const tabLabelPricingAndStock =
			"EditListingWizard.tabLabelPricingAndStock";
	...
	it("Negotiation: new listing flow with only details and style panels", async () => {
		...
		await waitFor(() => {
			...
			expect(
				getByRole("button", {
					name:
						"EditListingWizard.default-negotiation.new.saveDetails",
				})
			).toBeInTheDocument();
		});
...
describe("EditListingPageComponent", () => {
	it("Check that there is correct wizard tabs", async () => {
		...
		await act(async () => {
			userEvent.selectOptions(
				screen.getByLabelText(
					"EditListingDetailsForm.listingTypeLabel"
				),
				"product-selling"
			);
		});
		...
		const tabLabelAvailability = "EditListingWizard.tabLabelAvailability";
		expect(
			screen.queryByText(tabLabelAvailability)
		).not.toBeInTheDocument();
