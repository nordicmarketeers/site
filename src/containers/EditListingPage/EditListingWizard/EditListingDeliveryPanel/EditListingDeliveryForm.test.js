...
describe("EditListingDeliveryForm", () => {
	it("Check that shipping fees can be given and submit button activates", async () => {
		...
		expect(
			screen.getByRole("button", { name: saveActionMsg })
		).toBeDisabled();

		await act(async () => {
			// Add shipping price
			userEvent.click(
				screen.getByLabelText(/EditListingDeliveryForm.shippingLabel/i)
			);
		});
		const shippingOneItemLabel =
			"EditListingDeliveryForm.shippingOneItemLabel";
		const shippingAdditionalItemsLabel =
			"EditListingDeliveryForm.shippingAdditionalItemsLabel";
		userEvent.type(
			screen.getByRole("textbox", { name: shippingOneItemLabel }),
			"10"
		);
		userEvent.type(
			screen.getByRole("textbox", { name: shippingAdditionalItemsLabel }),
			"5"
		);
