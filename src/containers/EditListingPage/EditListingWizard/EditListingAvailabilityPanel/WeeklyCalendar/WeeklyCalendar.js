...
const DateLabel = ({ dateId, hasAvailability, timeZone }) => {
	...
	return (
		<div
			className={classNames(css.dateLabelContainer, {
				[css.blockedDateLabel]: !hasAvailability,
			})}
		>
...
const ExceptionEntry = ({
	...
	const millisecondBeforeEndTime = new Date(
		exception.attributes.end.getTime() - 1
	);
	const rangeEnd = isDaily
		? millisecondBeforeEndTime
		: exception.attributes.end;
	return (
...
const CalendarDate = props => {
	...
	return (
		<div
			className={classNames(css.date, {
				[css.blockedDate]: !hasAvailability,
			})}
		>
...
const WeeklyCalendar = props => {
	...
	const endOfCurrentWeek = getStartOf(
		currentWeek,
		"day",
		timeZone,
		6,
	...
	useEffect(() => {
		...
		if (
			...
			const formatRange = (start, end, available, timeZone) => {
				...
				const formattedStart = intl.formatDate(
					start,
					formattingOptions
				);
			...
			<header className={headerClassName}>
				<div className={css.titleWrapper}>
					...
					<WeekPicker
						...
						date={currentWeek}
						onDateChange={date => {
							const updatedDate = getStartOfWeek(
								date,
								timeZone,
								firstDayOfWeek
							);
						...
						startDateOffset={day => {
							return day != null
								? getStartOfWeek(day, null, firstDayOfWeek)
								: null;
						}}
						endDateOffset={day => {
							return day != null
								? getEndOfWeek(day, null, firstDayOfWeek)
								: null;
						}}
						isOutsideRange={day =>
							!isInRange(
				...
				<div className={css.navigation}>
					<Prev
						...
						startOfPrevRange={getStartOfPrevWeek(
							currentWeek,
							timeZone,
							firstDayOfWeek
						)}
						...
						aria-label={intl.formatMessage(
							{
								id:
									"EditListingAvailabilityPanel.WeeklyCalendar.screenreader.weekNavigation",
							},
							{ direction: "previous" }
						)}
					/>
					<Next
						...
						showUntilDate={endOfAvailabilityExceptionRange(
							timeZone,
							TODAY
						)}
						startOfNextRange={getStartOfNextWeek(
							currentWeek,
							timeZone,
							firstDayOfWeek
						)}
