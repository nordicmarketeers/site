...
const onExceptionStartDateChange = (value, availableDates, props) => {
	...
	const dayData =
		availableDates[stringifyDateToISO8601(selectedStartDate, timeZone)];
...
const onExceptionEndDateChange = (value, availableSlots, props) => {
	...
	const {
		exceptionStartDate: exceptionStart,
		exceptionStartTime: selectedStartTime,
	} = values;
...
const ExceptionDateTimeRange = props => {
		...
		if (appSettings.dev && appSettings.verbose) {
			...
			console.log(
				`Fetched months: ${monthIdString(
					startMonth
				)} - exclusive(${monthIdString(endMonth)})`,
				"\nExceptions found:",
				allExceptions
			);
	...
	const endTimeDisabled =
		!exceptionStartDate || !exceptionStartTime || !exceptionEndDate;
	const nextBoundary = findNextBoundary(TODAY, 1, "hour", timeZone);
	let placeholderTime = "08:00";
	try {
		placeholderTime = formatDateIntoPartials(nextBoundary, intl, {
			timeZone,
		})?.time;
	} catch (error) {
			...
			<div className={css.formRow}>
				...
				<div className={css.field}>
					<FieldSelect
						...
						selectClassName={
							exceptionStartDate ? css.select : css.selectDisabled
						}
			...
			<div className={css.formRow}>
				<div className={css.field}>
					<FieldSingleDatePicker
						...
						startDate={stringifyDateToISO8601(
							exceptionEndDay || exceptionStartDay || TODAY
						)}
						isOutsideRange={isOutsideRange(timeZone)}
						onChange={value =>
					...
					>
						{exceptionStartDay && exceptionStartTime && endDate ? (
							availableEndTimes.map((p, i) => {
								const isLastIndex =
									i === availableEndTimes.length - 1;
								const timeOfDay =
									p.timeOfDay === "00:00" && isLastIndex
										? "24:00"
										: p.timeOfDay;
								return (
									<option
