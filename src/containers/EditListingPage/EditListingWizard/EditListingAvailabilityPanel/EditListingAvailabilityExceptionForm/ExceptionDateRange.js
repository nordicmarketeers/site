...
const parseFieldDateInput = (isDaily, timeZone) => v => {
	...
	const endDateForAPI =
		parsedEnd && isDaily
			? getExclusiveEndDate(parsedEnd, timeZone)
			: parsedEnd;
...
const isDayBlocked = params => day => {
	...
	const hasAvailabilityOnDay =
		dayData == null ? false : dayData?.slots?.length > 0;
	...
	if (!isDaily && exceptionStartDay) {
		...
		const startDayIdString = stringifyDateToISO8601(
			exceptionStartDay,
			timeZone
		);
		const startDayData = availableDates[startDayIdString];
		const startDayException =
			startDayData == null ? true : startDayData?.slots?.[0];
		...
		const exceptionEndsOnThisDay =
			end && isSameDay(localizedDay, end, timeZone);
...
const isOutsideRange = timeZone => day => {
	...
	const rangeEnd = getStartOf(
		rangeStart,
		"day",
		timeZone,
		MAX_RANGE_FOR_EXCEPTIONS,
		"days"
	);
...
const isBlockedBetween = (availableDates, isDaily, timeZone) => ([
	startDate,
	endDate,
]) => {
	...
	const dayData =
		availableDates[stringifyDateToISO8601(localizedStartDay, timeZone)];
	...
	const isExcludedEnd =
		!isDaily && isSameDay(localizedEndDay, slot.end, timeZone);
	const isBlockedBetween = !(
		isInRange(localizedEndDay, slot.start, slot.end) || isExcludedEnd
	);
...
const ExceptionDateRange = props => {
	...
	useEffect(() => {
		if (appSettings.dev && appSettings.verbose) {
			...
			console.log(
				`Fetched months: ${monthIdString(
					startMonth
				)} ... ${monthIdString(lastFetchedMonth)}`,
				"\nExceptions found:",
				allExceptions
			);
			...
			<div className={css.formRow}>
				<div className={css.field}>
					<FieldDateRangePicker
						...
						startDatePlaceholderText={intl.formatDate(
							TODAY,
						...
						endDatePlaceholderText={intl.formatDate(
							TODAY,
							dateFormattingOptions
						)}
						onFocusedInputChange={handleFocusedInputChange(
							setFocusedInput
						)}
