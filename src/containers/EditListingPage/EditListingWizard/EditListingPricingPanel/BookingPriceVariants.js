 ...
 */
export const getInitialValuesForPriceVariants = (
	props,
	isUsingBookingPriceVariations
) => {
	const { listing } = props;
	...
	const variants = hasPriceVariants
		? priceVariants.map(variant => {
				...
				const bookingLengthInMinutes = setDefault(
					variant.bookingLengthInMinutes,
					60
				);
				const bookingLengthInMinutesMaybe = isFixedUnitType
					? { bookingLengthInMinutes }
					: {};
				const priceInSubunits = setDefault(
				...
				return {
					...
					price: priceInSubunits
						? new Money(priceInSubunits, price.currency)
						: null,
				};
		  })
		: isFixedUnitType
...
export const handleSubmitValuesForPriceVariants = (
	...
	const shouldIncludeName =
		isPriceVariationsInUse || priceVariants?.length > 1;
	...
	const lowestPrice = priceVariants.reduce((priceInSubUnits, variant) => {
		return priceInSubUnits < variant?.price?.amount
			? priceInSubUnits
...
const FieldBookingLength = props => {
	...
	const getHoursOptionLabel = hours =>
		intl.formatMessage(
			{ id: "EditListingPricingForm.priceVariant.hoursOption" },
			{ hours }
		);
	const getMinutesOptionLabel = minutes =>
		intl.formatMessage(
			{ id: "EditListingPricingForm.priceVariant.minutesOption" },
			{ minutes }
		);

	const toHourOption = (_, i) => ({ key: i, label: getHoursOptionLabel(i) });
	const optionsHours = Array.from(
		{ length: MAX_HOURS_FOR_BOOKING_DURATION },
		toHourOption
	);
	const toMinuteOption = minutes => ({
		key: minutes,
		label: getMinutesOptionLabel(minutes),
	});
		...
		<Field name={name} {...rest}>
			{({ input, meta }) => {
				const [factors, setFactors] = useState(
					getDurationFactors(input?.value)
				);
				const [hours, minutes] = factors;
				const { valid, invalid, touched, error } = meta;
				const handleHoursChange = e => {
					const newHours = e.target.value;
					const bookingLengthInMinutes = getDurationInMinutes(
						newHours,
						minutes
					);
				...
				return (
					<div className={classes}>
						<label>
							{intl.formatMessage({
								id:
									"EditListingPricingForm.priceVariant.bookingLengthLabel",
							})}
						</label>
...
const PriceVariant = props => {
	...
	const requiredValidator = validators.required(
		intl.formatMessage({
			id: "EditListingPricingForm.priceVariant.nameRequired",
		})
	);
		...
		<div className={css.priceVariant}>
			{isPriceVariationsInUse ? (
				<FieldTextInput
					...
					label={intl.formatMessage({
						id: "EditListingPricingForm.priceVariant.name",
					})}
					placeholder={intl.formatMessage({
						id:
							"EditListingPricingForm.priceVariant.nameInputPlaceholder",
					})}
					validate={validate}
				/>
			) : null}
			...
			<FieldCurrencyInput
				...
				label={intl.formatMessage(
					{
						id:
							"EditListingPricingForm.priceVariant.pricePerProduct",
					},
					{ unitType }
				)}
				placeholder={intl.formatMessage({
					id:
						"EditListingPricingForm.priceVariant.priceInputPlaceholder",
				})}
			...
			{unitType === FIXED ? (
				<FieldBookingLength
					...
					validate={validators.numberAtLeast(
						intl.formatMessage({
							id:
								"EditListingPricingForm.priceVariant.bookingLengthRequired",
						}),
						15
					)}
				/>
...
export const BookingPriceVariants = props => {
	...
	// This is needed because React virtual DOM needs to map with real DOM elements through unique keys.
	// https://github.com/final-form/react-final-form-arrays/issues/116
	const [data, setVariantKeys] = useState(
		initVariantKeys(props.initialLengthOfPriceVariants)
	);
		...
		<FieldArray
			...
			validate={validators.composeValidators(
				validators.nonEmptyArray(
					intl.formatMessage({
						id: "EditListingPricingForm.priceVariant.required",
					})
				)
			)}
		>
			{({ fields }) => {
				const priceVariantNames = fields?.value?.map(
					field => field.name
				);
				return (
					<>
						{fields?.length === 0 ? (
							<div className={css.noPriceVariant}>
								<div className={css.error}>
									{intl.formatMessage({
										id:
											"EditListingPricingForm.priceVariant.required",
									})}
								</div>
								<InlineTextButton
						...
						<div className={css.priceVariants}>
							...
							{areNamesUnique(priceVariantNames) ? null : (
								<div className={css.smallError}>
									{intl.formatMessage({
										id:
											"EditListingPricingForm.priceVariant.variationNamesMustBeUnique",
									})}
								</div>
							)}
						</div>
