...
const tabsForListingType = (processName, listingTypeConfig) => {
	...
	const deliveryMaybe =
		...
		displayDeliveryShipping(listingTypeConfig)
			? [DELIVERY]
			: [];
	const styleOrPhotosTab = requireListingImage(listingTypeConfig)
		? [PHOTOS]
		: [STYLE];
	...
	// Style and photos tab commented out
	// Card style insert in EditListingLocationPanel
	const tabs = {
		["default-booking"]: [
			DETAILS,
			...locationMaybe,
...
const getListingTypeConfig = (listing, selectedListingType, config) => {
	...
	const listingTypeConfig = existingListingType
		? validListingTypes.find(
				conf => conf.listingType === existingListingType
		  )
		: selectedListingType
		? validListingTypes.find(
...
class EditListingWizard extends Component {
	...
	handlePublishListing(id) {
		...
		const processName = listing?.attributes?.publicData?.transactionProcessAlias.split(
			"/"
		)[0];
