...
export const requestShowListing = (actionPayload, config) => (
	dispatch,
	getState,
	sdk
) => {
...
export const compareAndSetStock = (listingId, oldTotal, newTotal) => (
	dispatch,
	getState,
	sdk
) => {
...
// Backward compatible wrappers for the thunks
export const requestCreateListingDraft = (data, config) => (
	dispatch,
	getState,
	sdk
) => {
...
export const updateListingThunk = createAsyncThunk(
	...
	(
		...
		return updateStockOfListingMaybe(id, stockUpdate, dispatch)
			.then(() =>
				sdk.ownListings.update(ownListingUpdateValues, queryParams)
			)
			.then(response => {
				const state = getState();
				const existingTimeZone =
					state.marketplaceData.entities.ownListing[id.uuid]
				...
				if (
					!!includedTimeZone &&
					includedTimeZone !== existingTimeZone
				) {
					const searchString = "";
...
// Backward compatible wrappers for the thunks
export const requestUpdateListing = (tab, data, config) => (
	dispatch,
	getState,
	sdk
) => {
...
export const uploadImageThunk = createAsyncThunk(
	...
	(
		{ actionPayload, listingImageConfig },
...
export const requestImageUpload = (actionPayload, listingImageConfig) => (
	...
	return dispatch(
		uploadImageThunk({ actionPayload, listingImageConfig })
	).unwrap();
};
...
///////////////////////////////
const addAvailabilityExceptionPayloadCreator = (
	{ params },
	{ rejectWithValue, extra: sdk }
) => {
...
// Backward compatible wrappers for the thunks
export const requestAddAvailabilityException = params => (
	dispatch,
	getState,
	sdk
) => {
...
//////////////////////////////////
const deleteAvailabilityExceptionPayloadCreator = (
	{ params },
	{ rejectWithValue, extra: sdk }
) => {
	return sdk.availabilityExceptions
...
// Backward compatible wrappers for the thunks
export const requestDeleteAvailabilityException = params => (
	dispatch,
	getState,
	sdk
) => {
...
const fetchAvailabilityExceptionsPayloadCreator = (
	...
	return sdk.availabilityExceptions
		.query(fetchParams)
		.then(response => {
			const availabilityExceptions = denormalisedResponseEntities(
				response
			);
			...
			if (totalPages > 1 && !page) {
				...
				Promise.all(
					...
					const exceptions = responses.reduce(
						denormalizedFlatResults,
						[]
					);
					dispatch(
...
// Backward compatible wrappers for the thunks
export const requestFetchAvailabilityExceptions = params => (
	dispatch,
	getState,
	sdk
) => {
...
const fetchLoadDataExceptions = (dispatch, listing, search, firstDayOfWeek) => {
	...
	if (hasWindow && listing.id && hasTimeZone) {
		...
		const nextAfterNextWeek = getStartOf(
			nextWeek,
			"day",
			timezone,
			7,
	...
	extraReducers: builder => {
		builder
			...
			.addCase(createListingDraftThunk.fulfilled, (state, action) => {
				const updatedImagesState = updateUploadedImagesState(
					state,
					action.payload.data
				);
				state.uploadedImages = updatedImagesState.uploadedImages;
				state.uploadedImagesOrder =
					updatedImagesState.uploadedImagesOrder;
			...
			.addCase(uploadImageThunk.pending, (state, action) => {
				...
				state.uploadedImages = uploadedImages;
				state.uploadedImagesOrder = state.uploadedImagesOrder.concat([
					id,
				]);
				state.uploadImageError = null;
			})
			...
			.addCase(uploadImageThunk.rejected, (state, action) => {
				...
				state.uploadedImagesOrder = state.uploadedImagesOrder.filter(
					i => i !== id
				);
			...
			.addCase(
				fetchAvailabilityExceptionsThunk.pending,
				(state, action) => {
					const {
						isWeekly,
						start,
					...
					const weekStartId = isWeekly
						? stringifyDateToISO8601(start)
						: null;
					const monthId = !isWeekly
						? monthIdString(start, timeZone)
						: null;
					const newData = {
						fetchExceptionsError: null,
			...
			.addCase(
				fetchAvailabilityExceptionsThunk.fulfilled,
				(state, action) => {
					const { exceptions, monthId, weekStartId } = action.payload;
					const combinedExceptions = state.allExceptions.concat(
						exceptions
					);
					const selectId = x => x.id.uuid;
					state.allExceptions = uniqueBy(
						combinedExceptions,
						selectId
					).sort(sortExceptionsByStartTime);
			...
			.addCase(
				fetchAvailabilityExceptionsThunk.rejected,
				(state, action) => {
			...
			.addCase(
				addAvailabilityExceptionThunk.fulfilled,
				(state, action) => {
					const exception = action.payload.data;
					const combinedExceptions = state.allExceptions.concat(
						exception
					);
					state.allExceptions = combinedExceptions.sort(
			...
			.addCase(
				addAvailabilityExceptionThunk.rejected,
				(state, action) => {
			...
			.addCase(
				deleteAvailabilityExceptionThunk.fulfilled,
				(state, action) => {
					const exception = action.payload.data;
					const id = exception.id.uuid;
					state.allExceptions = state.allExceptions.filter(
						e => e.id.uuid !== id
					);
			...
			.addCase(
				deleteAvailabilityExceptionThunk.rejected,
				(state, action) => {
