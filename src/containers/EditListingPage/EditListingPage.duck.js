...
export const createListingDraftThunk = createAsyncThunk(
  ...
  ({ data, config }, { dispatch, getState, rejectWithValue, extra: sdk }) => {
    ...
    data.publicData.approved =
      data.publicData.listingType === 'consultant_profile' && isUnauthedConsultant(cu)
        ? 'pending'
        : 'approved';
...
const publishListingPayloadCreator = async (
  ...
  try {
    const publishResponse = await sdk.ownListings.publishDraft({ id: listingId }, { expand: true });
