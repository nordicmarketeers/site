...
const CTAButtonMaybe = props => {
	...
	const ctaLink = generateLinkProps(
		type,
		href,
		routeConfiguration,
		userId,
...
export const ActionBarMaybe = props => {
	...
	const approvalToPublishOptions =
		config?.accessControl?.listings?.requireApprovalToPublishOptions
			?.callToAction || {};
	...
	if (isOwnListing && showNoPayoutDetailsSet) {
			...
			<div className={classes}>
				<p
					className={classNames(
						css.ownListingText,
						css.missingPayoutDetailsText
					)}
				>
					<FormattedMessage id="ListingPage.addPayoutDetailsMessage" />
				</p>
				<NamedLink
					className={css.addPayoutDetails}
		...
		if (isPendingApproval) {
			ownListingTextTranslationId =
				"ListingPage.ownListingPendingApproval";
		} else if (isClosed) {
		...
		const hasValidType =
			approvalToPublishOptions?.type &&
			approvalToPublishOptions.type !== "none";
		const isCTAEnabled = hasValidType && isPendingApproval;
		...
		return (
			...
			>
				<p
					className={classNames(ownListingTextClasses, {
						[css.CTAEnabled]: isCTAEnabled,
					})}
				>
					<FormattedMessage id={ownListingTextTranslationId} />
				</p>
				<span
					className={isCTAEnabled ? css.linkContainer : css.noShrink}
				>
					<NamedLink
						className={classNames(css.editListingLink, {
							[css.CTAEnabled]: isCTAEnabled,
						})}
