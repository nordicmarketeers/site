...
export const fetchTransactionLineItems = ({
	...
	return dispatch(
		fetchTransactionLineItemsThunk({ orderData, listingId, isOwnListing })
	).unwrap();
};
	...
	extraReducers: builder => {
		builder
			...
			.addCase(fetchTimeSlotsThunk.pending, (state, action) => {
				...
				if (useFetchTimeSlotsForDate) {
					const dateId = stringifyDateToISO8601(start, timeZone);
					state.timeSlotsForDate = removeOutdatedDateData(
			...
			.addCase(fetchTimeSlotsThunk.rejected, (state, action) => {
				...
				if (useFetchTimeSlotsForDate) {
					...
					state.timeSlotsForDate[dateId].fetchTimeSlotsError =
						action.payload;
				} else {
					const monthId = monthIdString(start, timeZone);
					if (!state.monthlyTimeSlots[monthId]) {
					...
					state.monthlyTimeSlots[monthId].fetchTimeSlotsError =
						action.payload;
				}
			})
			...
			.addCase(
				fetchTransactionLineItemsThunk.fulfilled,
				(state, action) => {
			...
			.addCase(
				fetchTransactionLineItemsThunk.rejected,
				(state, action) => {
					state.fetchLineItemsInProgress = false;
