...
const generateCategories = optionStrings => {
	return optionStrings.reduce((converted, entry) => {
		...
		const option = isArray
			? {
					id: entry[0],
					name: labelize(entry[0]),
					subcategories: generateCategories(entry[1]),
			  }
...
const categories = generateCategories([
	...
	[
		"fish",
		[["freshwater", ["grayling", "arctic-char", "pike"]], "saltwater"],
	],
	["birds", ["parrot", "macaw"]],
]);
...
const listing1 = createListing(
	id,
	{ publicData },
	{ author: createUser("user-1") }
);
const listing1Own = createOwnListing(
	id,
	{},
	{ author: createCurrentUser("user-1") }
);
...
describe("ListingPage variants", () => {
	...
	it("has hero section in coverPhoto mode", async () => {
		...
		const listingRouteConfig = routeConfiguration.find(
			conf => conf.name === "ListingPage"
		);
		...
		await waitFor(() => {
			...
			expect(
				getByPlaceholderText("TopbarSearchForm.placeholder")
			).toBeInTheDocument();
			...
			const orderTitle = queryAllByRole("heading", {
				name: "ListingPage.orderTitle",
			});
			expect(orderTitle).toHaveLength(3);

			// Has details section title and selected category info
			expect(
				getByRole("heading", { name: "ListingPage.detailsTitle" })
			).toBeInTheDocument();
			...
			expect(
				getByRole("heading", { name: "ListingPage.locationTitle" })
			).toBeInTheDocument();

			// Has details reviews title
			const reviewsTitle = getByRole("heading", {
				name: "ListingPage.reviewsTitle",
			});
			expect(reviewsTitle).toBeInTheDocument();
			const sectionReviews = within(reviewsTitle.parentNode.parentNode);
			expect(
				sectionReviews.getByText("It was awesome!")
			).toBeInTheDocument();
			expect(
				sectionReviews.getByText("reviewerA display name")
			).toBeInTheDocument();
			...
			expect(
				getByRole("link", { name: "UserCard.viewProfileLink" })
			).toBeInTheDocument();
			// Has button to contact provider
			expect(
				getByRole("button", { name: "UserCard.contactUser" })
			).toBeInTheDocument();
		});
	});
...
describe("Duck", () => {
	...
	describe("showListing thunk", () => {
		it("should dispatch success and fetch current user", () => {
			...
			const store = configureStore({
				initialState: {
					ListingPage: reducer(undefined, { type: "@@INIT" }),
				},
				sdk,
			...
			return showListing(id, config)(dispatch, getState, sdk).then(
				data => {
					expect(sdk.listings.show.mock.calls).toEqual([
						[
							expect.objectContaining({
								id,
								"imageVariant.listing-card":
									"w:400;h:400;fit:crop",
								"imageVariant.listing-card-2x":
					...
					expect(relevantActions[0].type).toBe(
						"ListingPage/showListing/pending"
					);
					expect(relevantActions[0].meta.arg).toEqual({
						listingId: id,
						config,
						isOwn: false,
					});

					expect(relevantActions[1].type).toBe(
						"marketplaceData/addEntities"
					);
					expect(relevantActions[2].type).toBe(
					...
					const fetchCurrentUserFulfilled = actions.find(
						action =>
							action.type === "user/fetchCurrentUser/fulfilled"
					);
		...
		it("should dispatch error", () => {
			...
			return showListing(id, config)(dispatch, getState, sdk).catch(
				() => {
					...
					expect(relevantActions[1].type).toBe(
						"ListingPage/showListing/rejected"
					);
					expect(relevantActions[1].payload).toEqual(
						storableError(error)
					);
				}
			);
	...
	const fakeResponse = resource => ({
		data: { data: resource, include: [] },
	});
	...
	it("loadData() for currentUser with full viewing rights loads someone else's listing", () => {
		...
		return loadData({ id }, null, config)(dispatch, getState, sdk).then(
			data => {
				const relevantActions = actions.filter(
				...
				expect(relevantActions[2].type).toBe(
					"ListingPage/fetchReviews/pending"
				);
				expect(relevantActions[3]).toEqual(
					addMarketplaceEntities(
						fakeResponse(listing1),
						sanitizeConfig
					)
				);
				expect(relevantActions[4].type).toBe("auth/authInfo/pending");
				expect(relevantActions[5].type).toBe(
					"ListingPage/showListing/fulfilled"
				);
				expect(relevantActions[6].type).toBe(
	...
	it("loadData() for currentUser with no viewing rights does not load someone else's listing", () => {
		...
		// loadData() function is called. If you make customizations to the loadData() logic,
		// update the dispatched actions list in this test accordingly!
		return loadData({ id }, null, config)(dispatch, getState, sdk).catch(
			e => {
				...
				expect(relevantActions[3].type).toBe(
					"ListingPage/showListing/rejected"
				);
		...
		return loadData({ id }, null, config)(dispatch, getState, sdk).then(
			data => {
				...
				expect(relevantActions[2]).toEqual(
					addMarketplaceEntities(
						fakeResponse(listing1Own),
						sanitizeConfig
					)
				);
				expect(relevantActions[3].type).toBe("auth/authInfo/pending");
				expect(relevantActions[4].type).toBe(
					"ListingPage/showListing/fulfilled"
				);
...
describe("ActionBarMaybe", () => {
	...
	it("shows users own pending listing status", () => {
		...
		expect(
			screen.getByText("ListingPage.ownListingPendingApproval")
		).toBeInTheDocument();
		expect(screen.getByText("ListingPage.editListing")).toBeInTheDocument();
	});
	...
	it("shows users own closed listing status", () => {
		...
		expect(
			screen.getByText("ListingPage.ownClosedListing")
		).toBeInTheDocument();
		expect(screen.getByText("ListingPage.editListing")).toBeInTheDocument();
	});
	...
	it("shows closed listing status", () => {
		...
		expect(
			screen.getByText("ListingPage.closedListing")
		).toBeInTheDocument();
	});
