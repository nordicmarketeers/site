...
const checkOnetimePaymentFields = (
	...
	const billingDetailsKnown =
		hasHandledCardPayment || useDefaultPaymentMethod;
...
class StripePaymentForm extends Component {
			...
			window.addEventListener("resize", () => {
				if (this.card) {
					if (window.innerWidth < 768) {
						this.card.update({
							style: {
								base: { fontSize: "14px", lineHeight: "24px" },
							},
						});
					} else {
						this.card.update({
							style: {
								base: { fontSize: "18px", lineHeight: "24px" },
							},
						});
					}
				}
	...
	updateBillingDetailsToMatchShippingAddress(shouldFill) {
		...
		formApi.batch(() => {
			...
			formApi.change(
				"country",
				shouldFill ? values.recipientCountry : ""
			);
		});
	}
								...
								{askShippingDetails ? (
									<FieldCheckbox
										...
										label={intl.formatMessage({
											id:
												"StripePaymentForm.sameBillingAndShippingAddress",
										})}
										value="sameAddress"
										useSuccessColor
