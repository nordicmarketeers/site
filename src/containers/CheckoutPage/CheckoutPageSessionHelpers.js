...
export const isValidListing = listing => {
	...
	const props = {
		...
		attributes: v => {
			return (
				typeof v === "object" &&
				(isNegotiation || (!isNegotiation && v.price instanceof Money))
			);
		},
	};
...
export const isValidTransaction = transaction => {
	...
	const props = {
		...
		attributes: v => {
			return (
				typeof v === "object" &&
				Object.values(process.transitions).includes(v.lastTransition)
			);
		},
	};
	...
	if (window && window.sessionStorage) {
		...
		const reviver = (k, v) => {
			if (
				v &&
				typeof v === "object" &&
				v._serializedType === "SerializableDate"
			) {
				...
				v &&
				typeof v === "object" &&
				v._serializedType === "SerializableDecimal"
			) {
