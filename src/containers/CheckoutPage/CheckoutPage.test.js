...
describe("CheckoutPage", () => {
	...
	it("Check that purchase has relevant info", () => {
		const listing = createListing(
			...
			},
			{
				...
				images: [createImage("first-image")],
			}
		);
		...
		expect(
			screen.getByRole("heading", { name: purchaseTitle })
		).toBeInTheDocument();
		const orderBreakdownTitle =
			"CheckoutPage.default-purchase.orderBreakdown";
		expect(
			screen.getByRole("heading", { name: orderBreakdownTitle })
		).toBeInTheDocument();
		// These are rendered twice (mobile & desktop)
		expect(
			screen.getAllByText("OrderBreakdown.baseUnitQuantity")
		).toHaveLength(2);
		expect(screen.getAllByText("OrderBreakdown.shippingFee")).toHaveLength(
			2
		);
		...
		expect(
			screen.getByRole("heading", { name: shippingHeading })
		).toBeInTheDocument();
		expect(
			getTextbox("ShippingDetails.recipientNameLabel")
		).toBeInTheDocument();
		expect(
			getTextbox("ShippingDetails.recipientPhoneNumberLabel")
		).toBeInTheDocument();
		expect(
			getTextbox("ShippingDetails.addressLine1Label")
		).toBeInTheDocument();
		expect(
			getTextbox("ShippingDetails.addressLine2Label")
		).toBeInTheDocument();
		expect(
			getTextbox("ShippingDetails.postalCodeLabel")
		).toBeInTheDocument();
		...
		expect(
			screen.getByRole("heading", { name: paymentHeading })
		).toBeInTheDocument();

		const billingDetails = "StripePaymentForm.billingDetails";
		expect(
			screen.getByRole("heading", { name: billingDetails })
		).toBeInTheDocument();
		expect(
			getTextbox("StripePaymentForm.billingDetailsNameLabel")
		).toBeInTheDocument();
		expect(
			getTextbox("StripePaymentAddress.addressLine1Label")
		).toBeInTheDocument();
		expect(
			getTextbox("StripePaymentAddress.addressLine2Label")
		).toBeInTheDocument();
		expect(
			getTextbox("StripePaymentAddress.postalCodeLabel")
		).toBeInTheDocument();
		expect(
			getTextbox("StripePaymentAddress.cityLabel")
		).toBeInTheDocument();
		expect(
			getTextbox("StripePaymentAddress.stateLabel")
		).toBeInTheDocument();
		expect(
			getTextbox("StripePaymentForm.messageLabel")
		).toBeInTheDocument();

		expect(
			screen.getByRole("button", {
				name: "StripePaymentForm.submitPaymentInfo",
			})
		).toBeInTheDocument();
	});
	...
	it("Check that booking has relevant info", () => {
		...
		expect(
			screen.getByRole("heading", { name: bookingTitle })
		).toBeInTheDocument();
		const orderBreakdownTitle =
			"CheckoutPage.default-booking.orderBreakdown";
		expect(
			screen.getByRole("heading", { name: orderBreakdownTitle })
		).toBeInTheDocument();
		...
		expect(
			screen.queryAllByText("OrderBreakdown.shippingFee")
		).toHaveLength(0);
		expect(screen.getAllByText("OrderBreakdown.total")).toHaveLength(2);
	...
	it("Check that inquiry process has relevant info", () => {
		...
		expect(
			screen.getByRole("heading", { name: inquiryTitle })
		).toBeInTheDocument();
		const inquiryMessageLabel =
			"CheckoutPageWithInquiryProcess.messageLabel";
		expect(
			screen.getByRole("heading", { name: inquiryMessageLabel })
		).toBeInTheDocument();
		// price
		...
		const orderBreakdownTitle =
			"CheckoutPage.default-inquiry.orderBreakdown";
		expect(
			screen.queryByRole("heading", { name: orderBreakdownTitle })
		...
		const paymentHeading = "StripePaymentForm.paymentHeading";
		expect(
			screen.queryByRole("heading", { name: paymentHeading })
		).not.toBeInTheDocument();

		const billingDetails = "StripePaymentForm.billingDetails";
		expect(
			screen.queryByRole("heading", { name: billingDetails })
		).not.toBeInTheDocument();
	...
	describe("Duck", () => {
		it("ActionCreator: setInitialValues(initialValues)", () => {
			...
			expect(setInitialValues({ listing, orderData })).toEqual(
				expectedAction
			);
		});
		...
		describe("Reducer", () => {
			...
			it("should return the initial state", () => {
				expect(checkoutPageReducer(undefined, {})).toEqual(
					initialValues
				);
			});
			...
			it("should handle setInitialValues", () => {
				...
				expect(
					checkoutPageReducer(
						{},
						{ type: "CheckoutPage/setInitialValues", payload }
					)
				).toEqual(expected);
			});
		});
