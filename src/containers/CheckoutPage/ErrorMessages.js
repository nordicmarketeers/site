...
export const getErrorMessages = (
	...
	if (listingNotFound) {
		...
		isTransactionInitiateListingInsufficientStockError(initiateOrderError)
	) {
		// If stock management is used, there could be error related to that
		initiateOrderErrorMessage = (
			<FormattedMessage id="CheckoutPage.notEnoughStockMessage" />
		);
		...
		if (
			stripeErrors.length === 1 &&
			stripeErrors[0].indexOf(noAccountMsg) > -1
		) {
			initiateOrderErrorMessage = (
	...
	if (
		...
		isTransitionQuantityInfoMissingError(speculateTransactionError)
	) {
		speculateErrorMessage = (
			<FormattedMessage id="CheckoutPage.correctQuantityInformationMissing" />
		);
	} else if (
		isProviderCommissionBiggerThanMinPrice(speculateTransactionError)
	) {
		speculateErrorMessage = (
			<FormattedMessage id="CheckoutPage.providerCommissionBiggerThanMinPrice" />
		);
