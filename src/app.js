...
const MomentLocaleLoader = props => {
	...
	const MomentLocale =
		["en", "en-US"].includes(locale) || isAlreadyImportedLocale
			...
			: loadable.lib(() =>
					import(
						/* webpackChunkName: "locales" */ "moment/min/locales.min"
					)
			  );
	...
	return (
		...
		>
			<div style={{ width: "600px" }}>
				<p>
					Are you sure you want to reveal to the public web an
					environment variable called: <b>{suspiciousEnvKey}</b>
				</p>
				<p>
					All the environment variables that start with{" "}
					<i>REACT_APP_</i> prefix will be part of the published React
					app that's running on a browser. Those variables are,
					therefore, visible to anyone on the web. Secrets should only
					be used on a secure environment like the server.
				</p>
				{containsINTEG(suspiciousEnvKey) ? (
					<p>
						{"Note: "}
						<span style={{ color: "red" }}>
							Do not use Integration API directly from the web
							app.
						</span>
					</p>
				) : null}
		...
		if (suspiciousSECRETKey) {
			return (
				<EnvironmentVariableWarning
					suspiciousEnvKey={suspiciousSECRETKey}
				/>
			);
		}
	}
