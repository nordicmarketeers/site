...
class Handle extends Component {
	...
	componentDidMount() {
		...
		this.onMouseMoveListener = window.addEventListener(
			"mousemove",
			this.onMouseMove,
			false
		);
		this.onMouseUpListener = window.addEventListener(
			"mouseup",
			this.onMouseUp,
			false
		);
	...
	onMove(pagePosition) {
		...
		const value =
			currentValue < min ? min : currentValue > max ? max : currentValue;
	...
	render() {
		...
		const classes = classNames(
			rootClassName || css.rootTouchBuffer,
			className
		);
