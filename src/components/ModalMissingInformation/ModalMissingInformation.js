...
class ModalMissingInformation extends Component {
	constructor(props) {
		...
		this.handleMissingInformationReminder = this.handleMissingInformationReminder.bind(
			this
		);
	}
	...
	handleMissingInformationReminder(
		...
		const isPageWhitelisted = whitelistedPaths.includes(
			newLocation.pathname
		);

		// Track if path changes inside Page level component
		const pathChanged =
			newLocation.pathname !== this.props.location.pathname;
		...
		if (!isPageWhitelisted && showOnPathChange) {
			...
			const emailVerificationNeeded =
				hasListingsOrOrders && emailUnverified;
