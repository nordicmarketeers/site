...
const NegotiationForm = loadable(() =>
	import(
		/* webpackChunkName: "NegotiationForm" */ "./NegotiationForm/NegotiationForm"
	)
);
...
const OrderPanel = props => {
	...
	const shouldHaveBookingDates =
		isBooking &&
		[LINE_ITEM_DAY, LINE_ITEM_NIGHT].includes(lineItemUnitType);
	const showBookingDatesForm =
		mounted && shouldHaveBookingDates && !isClosed && timeZone;

	// The listing resource has a relationship: `currentStock`,
	// which you should include when making API calls.
	const shouldHavePurchase =
		isPurchase && lineItemUnitType === LINE_ITEM_ITEM;
	...
	const showNegotiationForm =
		mounted && !isClosed && isNegotiation && unitType === REQUEST;
	// if listing is an offer, we show the "request a quote" form as user needs to ask for a quote first from the provider.
	const showRequestQuoteForm =
		mounted && !isClosed && isNegotiation && unitType === OFFER;
	...
	const preselectedPriceVariant =
		...
		isPriceVariationsInUse
			? priceVariants.find(
					pv =>
						pv?.name &&
						createSlug(pv?.name) === preselectedPriceVariantSlug
			  )
			: null;
		...
		<div className={classes}>
			<ModalInMobile
				...
				onClose={() => {
					...
					document
						.getElementById(ORDER_PANEL_SUBMIT_BUTTON_ID)
						?.focus();
				}}
