	...
	return hasMultiplePriceVariants ? (
		<FieldSelect
			...
			label={intl.formatMessage({
				id: "PriceVariantPicker.priceVariantLabel",
			})}
		...
		>
			<option disabled value="" key="unselected">
				{intl.formatMessage({
					id: "PriceVariantPicker.priceVariantUnselected",
				})}
			</option>
		...
		<>
			<VariantNameMaybe
				...
				className={css.priceVariantName}
			/>
			<FieldHidden
				name="priceVariantName"
				format={value => {
				...
				parse={value => {
					const response =
						value === DEFAULT_PRICE_VARIANT_NAME ? null : value;
					return response;
				}}
			/>
		</>
