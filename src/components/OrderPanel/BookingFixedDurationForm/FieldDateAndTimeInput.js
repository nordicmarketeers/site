...
const getAvailableStartTimes = params => {
	...
	const allStartTimes = timeSlotsOnSelectedDate.reduce(
		(availableStartTimes, t, i) => {
			const startDate = t.attributes.start;
			const lastIndex = findLastAdjacent(i, timeSlotsOnSelectedDate);
			const endDate =
				lastIndex !== i
					? timeSlotsOnSelectedDate[lastIndex].attributes.end
					: t.attributes.end;
			...
			const endOfTimeSlotOrDay = isDateSameOrAfter(
				endDate,
			...
			const uniqueStartTimes = startTimes.filter(
				t => !pickedTimestamps.includes(t.timestamp)
			);
...
const onBookingStartDateChange = (props, setCurrentMonth) => value => {
	...
	const nextBoundaryToday = findNextBoundary(
		new Date(),
		1,
		timeUnit,
		timeZone
	...
	const cachedTimeSlotsForDate =
		timeSlotsForDate[stringifyDateToISO8601(startDate, timeZone)]
			?.timeSlots || [];
...
const FieldDateAndTimeInput = props => {
	...
	const seatsOptions = selectedTimeSlot?.attributes?.seats
		? Array.from(
				{ length: selectedTimeSlot.attributes.seats },
				(_, i) => i + 1
		  )
		: [];
	...
	useEffect(() => {
		...
		if (
			...
			const options = {
				minDurationStartingInDay: minDurationStartingInInterval,
			};
			const monthlyTimeSlotsData = timeSlotsPerDate(
				currentMonth,
				nextMonth,
			...
			console.log(
				`Fetched months: ${monthIdString(
					startMonth,
					tz
				)} ... ${monthIdString(lastFetchedMonth, tz)}`,
			...
			<div className={css.formRow}>
				<div className={classNames(css.field, css.startDate)}>
					<FieldSingleDatePicker
						...
						validate={bookingDateRequired(
							intl.formatMessage({
								id: "BookingFixedDurationForm.requiredDate",
							})
						)}
						...
						fallback={
							<div
								className={classNames(css.fieldDatePicker, {
							...
							>
								...
								<input
									className={classNames(
										css.fieldDateInput,
										css.fieldDateInputFallback
									)}
									placeholder={
										startDateInputProps.placeholderText
									}
									disabled={disabled}
								/>
							</div>
				...
				<div className={classNames(css.field, css.startTime)}>
					<FieldSelect
						...
						selectClassName={
							bookingStartDate ? css.select : css.selectDisabled
						}
