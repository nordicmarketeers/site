...
describe("OrderPanel", () => {
	...
	const validListingTypes = config.listingTypes.listingTypes.map(
		({ id, ...rest }) => ({
			listingType: id,
			transactionType: {
	...
	it("Booking: daily", async () => {
		...
		await waitFor(() => {
			...
			expect(queryAllByText("OrderPanel.priceInMobileCTA")).toHaveLength(
				1
			);
			...
			expect(
				getByText("OrderPanel.youWontBeChargedInfo")
			).toBeInTheDocument();
			expect(
				getByText("OrderPanel.ctaButtonMessageBooking")
			).toBeInTheDocument();
		});
	});
	...
	it("Booking: fixed", async () => {
		...
		await waitFor(() => {
			...
			expect(queryAllByText("OrderPanel.author")).toHaveLength(2);
			expect(
				getByText("BookingFixedDurationForm.bookingStartTitle")
			).toBeInTheDocument();
			expect(
				getByText("FieldDateAndTimeInput.startTime")
			).toBeInTheDocument();
			expect(
				queryByText("FieldDateAndTimeInput.endTime")
			).not.toBeInTheDocument();
			expect(
				getByText("BookingFixedDurationForm.requestToBook")
			).toBeInTheDocument();
			expect(
				getByText("OrderPanel.youWontBeChargedInfo")
			).toBeInTheDocument();
	...
	it("Purchase: item ", async () => {
		...
		await waitFor(() => {
			...
			expect(queryAllByText("OrderPanel.author")).toHaveLength(2);
			expect(
				getByText("ProductOrderForm.quantityLabel")
			).toBeInTheDocument();
			expect(
				getByText("ProductOrderForm.deliveryMethodLabel")
			).toBeInTheDocument();
			expect(
				getByText("ProductOrderForm.pickupOption")
			).toBeInTheDocument();
			expect(
				getByText("ProductOrderForm.breakdownTitle")
			).toBeInTheDocument();
	...
	it("Purchase: item (no delivery method set)", async () => {
		...
		await waitFor(() => {
			...
			expect(queryAllByText("OrderPanel.author")).toHaveLength(2);
			expect(
				getByText("ProductOrderForm.noDeliveryMethodSet")
			).toBeInTheDocument();
			expect(
				getByText("OrderPanel.ctaButtonMessagePurchase")
			).toBeInTheDocument();
	...
	it("Inquiry: inquiry ", async () => {
		...
		await waitFor(() => {
			...
			expect(queryAllByText("OrderPanel.author")).toHaveLength(2);
			expect(
				getByText("InquiryWithoutPaymentForm.ctaButton")
			).toBeInTheDocument();
			expect(
				getByText("OrderPanel.ctaButtonMessageInquiry")
			).toBeInTheDocument();
		});
	});
});
