...
const getSupportedCountryCodes = supportedCountries =>
  ...
  */
	...
	useEffect(() => {
		...
		if (!hasBusinessURL && currentUserId) {
			...
			const rootUrl = hasMarketplaceRootURL
			...
			const profilePageURL =
				hasMarketplaceRootURL && !rootUrl.includes("localhost")
					...
					: `https://test-marketplace.com${pathToProfilePage(
							currentUserId.uuid
					  )}`;
	...
	return (
		<div className={css.savedInformation}>
			...
			<div className={css.savedCountry}>
				<FormattedMessage
					id={`StripeConnectAccountForm.accountTypes.${savedAccountType}`}
				/>
			</div>
			<label className={css.accountInformationTitle}>
				{countryLabel}
...
const ErrorsMaybe = props => {
	...
	const errorMessage = isStripeError(stripeAccountError) ? (
		<FormattedMessage
			...
			values={{
				stripeMessage:
					stripeAccountError.apiErrors[0].meta.stripeMessage,
			}}
		/>
	) : stripeAccountError ? (
		...
		<FormattedMessage
			...
			values={{
				stripeMessage:
					stripeAccountLinkError.apiErrors[0].meta.stripeMessage,
			}}
		/>
...
const StripeConnectAccountForm = props => {
	...
	return (
		<FinalForm
			{...restOfProps}
			onSubmit={values =>
				onSubmit({ ...values, stripePublishableKey }, isUpdate)
			}
			mutators={{
				...arrayMutators,
			}}
			render={fieldRenderProps => {
				...
				const accountDataLoaded =
					stripeConnected && stripeAccountFetched && savedCountry;
				...
				const submitButtonTitle = limitedRights
					? intl.formatMessage({
							id:
								"StripeConnectAccountForm.buttonTitleLimitedAccess",
					  })
					: null;
