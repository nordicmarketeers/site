...
class Menu extends Component {
	constructor(props) {
		...
		this.positionStyleForMenuContent = this.positionStyleForMenuContent.bind(
			this
		);
	...
	onKeyDown(e) {
		...
		if (e.key === "Escape") {
			...
			if (e.target === this.menu?.firstChild) {
				...
				this.toggleOpen({
					enforcedState: true,
					moveFocusFn: moveFocusToLastFocusableElement,
				});
			} else if (this.menuContent?.contains(e.target)) {
				e.preventDefault();
				e.stopPropagation();
	...
	positionStyleForMenuContent(contentPosition) {
		if (this.menu && this.menuContent) {
			...
			const contentWidthBiggerThanLabel =
				this.menuContent.offsetWidth - menuWidth;
			const usePositionLeftFromLabel =
				contentPosition === CONTENT_TO_LEFT;
			const contentPlacementOffset =
				this.props.contentPlacementOffset ?? CONTENT_PLACEMENT_OFFSET;
			const mobileMaxWidth =
				this.props.mobileMaxWidth || MAX_MOBILE_SCREEN_WIDTH;

			if (
				this.props.preferScreenWidthOnMobile &&
