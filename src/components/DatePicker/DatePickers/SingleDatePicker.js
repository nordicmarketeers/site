...
export const SingleDatePicker = props => {
	...
	const [dateData, setDateData] = useState({
		...
		formatted: props.value
			? intl.formatDate(props.value, dateFormatOptions)
			: "",
	});
	...
	const handleChange = value => {
		...
		setDateData({
			date: startOfDay,
			formatted: intl.formatDate(startOfDay, dateFormatOptions),
		});
		setIsOpen(false);
	...
	const handleOnChangeOnInput = e => {
		...
		if (isValidDateString(inputStr)) {
			...
			if (isDayBlocked(d)) {
				...
				setDateData({
					date: d,
					formatted: intl.formatDate(d, dateFormatOptions),
				});
				return;
			}
		}
