...
const stringify = arr =>
	arr ? arr.map(v => (v ? v.getTime() : "")).join(",") : "";
...
export const DateRangePicker = props => {
	...
	useEffect(() => {
		if (
			mounted &&
			stringify(value) !== stringify(dateRangeData.dateRange)
		) {
			// If mounted, changes to value should be reflected to 'dateRange' state
			setDateRangeData({
				dateRange: value,
				formatted: value.map(value =>
					intl.formatDate(value, dateFormatOptions)
				),
			});
		}
	...
	const handleChange = value => {
		...
		if (cleanedValues.length === 1) {
			setDateRangeData({
				...
				formatted: [
					intl.formatDate(cleanedValues[0], dateFormatOptions),
					"",
				],
			});
			...
			setDateRangeData({
				dateRange: cleanedValues,
				formatted: cleanedValues.map(value =>
					intl.formatDate(value, dateFormatOptions)
				),
			});
			...
			if (element.current) {
				const el =
					element.current.querySelector(`#${endDateId}`) ||
					document.getElementById(endDateId);
				el.focus();
			}
	...
	const handleOnChangeOnInput = e => {
		...
		if (!inputStr) {
			const newDateRange =
				...
				dateRangeData.formatted[1] &&
				dateRangeData.dateRange[1]
					? [dateRangeData.dateRange[1]]
					: inputType !== INPUT_START &&
					  dateRangeData.formatted[0] &&
			...
			if (onChange) {
				const newFormattedValues = getUpdatedRange(
					null,
					dateRangeData.formatted
				);
			...
			if (updatedRange?.[0] && updatedRange?.[1]) {
				if (isBlockedBetween(updatedRange)) {
					...
					setDateRangeData({
						...
						formatted: [
							intl.formatDate(updatedRange[0], dateFormatOptions),
							"",
						],
					});
				...
				<div className={popupClassName || css.popup}>
					{isOpen ? (
						<DatePicker
							...
							rangeStartHasValue={
								dateRangeData.formatted?.[0]?.length > 0
							}
							rangeEndHasValue={
								dateRangeData.formatted?.[1]?.length > 0
							}
							{...startDateMaybe}
							{...rest}
						/>
