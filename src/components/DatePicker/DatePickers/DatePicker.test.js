...
const intl = createIntl(
	{
		...
		messages: {
			...
			"DatePicker.screenreader.chooseEndDate":
				"Choose {date} as an end date",
			"DatePicker.screenreader.chooseStartDate":
				"Choose {date} as a start date",
			"DatePicker.screenreader.selectedDate": "{date} is selected",
			"DatePicker.screenreader.selectedEndDate":
				"{date} is selected as the end date",
			"DatePicker.screenreader.selectedStartDate":
				"{date} is selected as the start date",
...
describe("DatePicker", () => {
		...
		dates.map(date =>
			currentMonthCal.getByRole("button", {
				name: intl.formatMessage(
					{ id: "DatePicker.screenreader.chooseDate" },
					{ date }
				),
			})
		);
	...
	it("Shows configured start date", () => {
		...
		const currentMonthString = screen.getByText(
			`${
				getMonths(intl)[localStartDate.getMonth()]
	...
	it("Selects dates: June 15th, 25th, 26th, 19th", () => {
		...
		expect(
			screen.getByRole("button", { name: "June 15 is selected" })
		).toBeInTheDocument();
		userEvent.click(screen.getByRole("button", { name: "Choose June 25" }));
		expect(
			screen.getByRole("button", { name: "June 25 is selected" })
		).toBeInTheDocument();
		userEvent.keyboard("[ArrowRight][Enter]");
		expect(
			screen.getByRole("button", { name: "June 26 is selected" })
		).toBeInTheDocument();
		expect(
			screen.queryByRole("button", { name: "June 25 is selected" })
		).not.toBeInTheDocument();
		userEvent.keyboard("[ArrowUp][Enter]");
		expect(
			screen.getByRole("button", { name: "June 19 is selected" })
		).toBeInTheDocument();
		expect(
			screen.getByRole("button", { name: "June 19 is selected" })
		).toHaveClass("dateCurrent");
		expect(
			screen.getByRole("button", { name: "Choose June 15" })
		).not.toHaveClass("dateCurrent");
	});
	...
	it("Highlights current date with keyboard selection", () => {
		...
		render(<DatePicker {...{ ...props, startDate }} />);
		userEvent.click(screen.getByRole("button", { name: "Choose June 15" }));
		const clicked15 = screen.getByRole("button", {
			name: "June 15 is selected",
		});
		expect(clicked15).toBeInTheDocument();
		expect(clicked15).toHaveClass("dateCurrent");
		userEvent.keyboard("[ArrowRight][Enter]");
		const selected16 = screen.getByRole("button", {
			name: "June 16 is selected",
		});
	...
	it("disables dates", () => {
		...
		const dateString = intl.formatDate(disabledDay, {
			day: "numeric",
			month: "long",
		});
		...
		const disabledDateBtn = screen.getByRole("button", {
			name: `${dateString} is not available`,
		});
	...
	it("changes months through key navigation", async () => {
		const startDate = "2024-08-11";
		const day = removeTimezoneOffset(new Date(startDate));
		const dateString = intl.formatDate(day, {
			day: "numeric",
			month: "long",
		});
		...
		userEvent.click(
			screen.getByRole("button", { name: `Choose ${dateString}` })
		);
		userEvent.keyboard(
	...
	it("jumps to current month", async () => {
		...
		const getCurrentMonthString = (localDate, scr) =>
			scr.getByText(
				`${
					getMonths(intl)[localDate.getMonth()]
		...
		expect(currentMonthString).toBeInTheDocument();
		userEvent.click(
			screen.getByRole("button", { name: `DatePicker.todayButton` })
		);
		const currentMonthString2 = getCurrentMonthString(
	...
	it("clears its value", async () => {
		...
		userEvent.click(
			screen.getByRole("button", { name: `DatePicker.clearButton` })
		);
		expect(clicked15).toBeInTheDocument();
...
describe("SingleDatePicker", () => {
	...
	it("shows selected date", () => {
		...
		const calendarDateString = intl.formatDate(startDay, {
			day: "numeric",
			month: "long",
		});
		expect(
		...
		const inputDateFormatOptions = {
			day: "numeric",
			month: "short",
			weekday: "short",
		};
		const todayInputString = intl.formatDate(
			startDay,
			inputDateFormatOptions
		);
		...
		userEvent.keyboard("[ArrowRight][Enter]");
		const nextDayInputString = intl.formatDate(
			addDays(startDay, 1),
			inputDateFormatOptions
		);
		expect(
			screen.getByDisplayValue(nextDayInputString)
		).toBeInTheDocument();
		expect(
			screen.queryByDisplayValue(todayInputString)
		).not.toBeInTheDocument();
	});
});
...
describe("DateRangePicker", () => {
	...
	it("shows selected date range: single day with minimumNights: 0", () => {
		...
		const calendarStartDayString = intl.formatDate(startDay, {
			day: "numeric",
			month: "long",
		});
		expect(
		...
		const startDayInputString = intl.formatDate(
			startDay,
			inputDateFormatOptions
		);
		...
		const endDayInputString = intl.formatDate(
			endDay,
			inputDateFormatOptions
		);
		// These are the same
		expect(screen.queryAllByDisplayValue(startDayInputString)).toHaveLength(
			2
		);
		expect(screen.queryAllByDisplayValue(endDayInputString)).toHaveLength(
			2
		);
	...
	it("shows selected date range: minimumNights: 1", () => {
		const startDate = "2024-08-11";
		render(
			<DateRangePicker {...{ ...props, startDate, minimumNights: 1 }} />
		);
		...
		const faultyEndDayInputString = intl.formatDate(
			faultyEndDay,
			inputDateFormatOptions
		);
		expect(
			screen.getByDisplayValue(startDayInputString)
		).toBeInTheDocument();
		expect(
			screen.queryByDisplayValue(faultyEndDayInputString)
		).not.toBeInTheDocument();
		...
		const calendarEndDayString = intl.formatDate(endDay, {
			day: "numeric",
			month: "long",
		});
		expect(
