...
const locationBounds = (latlng, distance) => {
	...
	const bounds = new window.mapboxgl.LngLat(latlng.lng, latlng.lat).toBounds(
		distance
	);
...
const placeBounds = prediction => {
	if (prediction) {
		if (Array.isArray(prediction.bbox) && prediction.bbox.length === 4) {
			...
			const placeType =
				Array.isArray(prediction.place_type) &&
				prediction.place_type[0];

			const distance =
				(placeType && PLACE_TYPE_BOUNDS_DISTANCES[placeType]) ||
