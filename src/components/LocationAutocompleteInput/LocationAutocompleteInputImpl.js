...
class LocationAutocompleteInputImplementation extends Component {
	constructor(props) {
		...
		this.selectItemIfNoneSelected = this.selectItemIfNoneSelected.bind(
			this
		);
	...
	handlePredictionsSelectMove(touchCoordinates) {
		this.setState(prevState => {
			...
			const isSwipe = isTouchAction
				? Math.abs(touchStartedFrom.y - touchCoordinates.y) >
				  TOUCH_TAP_RADIUS
				: false;
			...
			<div className={rootClass}>
				...
				<input
					...
					aria-activedescendant={
						predictions[this.state.highlightedIndex]?.id
					}
				/>
				{renderPredictions ? (
