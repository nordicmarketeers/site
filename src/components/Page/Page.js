...
const getFaviconVariants = config => {
	...
	return favicon?.type === "imageAsset"
		? Object.values(favicon.attributes.variants)
		: [];
};
...
const getAppleTouchIconURL = config => {
	...
	const appIconVariants =
		appIcon?.type === "imageAsset"
			? Object.values(appIcon.attributes.variants)
			: [];
...
class PageComponent extends Component {
	constructor(props) {
		...
		this.scrollingDisabledChanged = this.scrollingDisabledChanged.bind(
			this
		);
	}
	...
	scrollingDisabledChanged(currentScrollingDisabled) {
		if (
			...
			currentScrollingDisabled !== this.scrollingDisabled
		) {
	...
	render() {
		...
		const facebookImgs =
			socialSharingImages1200 ||
			facebookImages ||
			openGraphFallbackImages;
		const twitterImgs =
			socialSharingImages600 || twitterImages || twitterFallbackImages;
		...
		const metaToHead = metaTagProps(
			{
				...
				socialSharingDescription:
					socialSharingDescription || pageDescription,
		...
		const schemaFromProps =
			hasSchema && Array.isArray(schema)
				? schema
				: hasSchema
				? [schema]
				: [];
		const addressMaybe = config.address?.streetAddress
		...
		return (
			<div id="page" className={classes} style={styles}>
				...
				>
					...
					<link
						rel="manifest"
						href={webmanifestURL(marketplaceRootURL)}
					/>
