...
const JobListingCard = props => {
  ...
  const authorImgURL = author.profileImage?.attributes.variants['square-small']?.url || noPfp;
      ...
      <div className={css.jobCardWrapper}>
        ...
        <div className={css.info}>
          ...
          <div className={css.tagRow}>
            {publicData?.role?.[0] && (
              <span className={classNames(css.tag, css.roleTag)}>{capitalize(displayRole)}</span>
            )}

            {restRoles.length > 0 && (
              <span
                className={classNames(css.tag, css.moreRolesTag)}
            ...
            {publicData?.work_model && (
              <span className={css.tag}>{capitalize(publicData.work_model)}</span>
            )}
            {publicData?.extent_job && (
              <span className={css.tag}>
                {capitalize(publicData.extent_job)}{' '}
                {publicData.part_time_percent && `(${publicData.part_time_percent}%)`}
              </span>
            )}
          </div>
        </div>
