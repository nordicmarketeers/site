...
const getNextOption = (currentButton, container) => {
	...
	const siblingButtons = Array.from(
		parentList.querySelectorAll("button")
	...
	const nextIndex =
		currentIndex === siblingButtons.length - 1 ? 0 : currentIndex + 1;
...
const getPreviousOption = (currentButton, container) => {
	...
	const previousIndex =
		currentIndex === 0 ? siblingButtons.length - 1 : currentIndex - 1;
...
const pickValidOptions = (options, level = 1) => {
	...
	return options.reduce((picked, optionConfig) => {
		...
		const suboptionsMaybe = isValidOptions(suboptions)
			...
			: {};
		const validOptionConfigMaybe = isValid
			? [{ option, label, level, ...suboptionsMaybe }]
			: [];
...
const Option = props => {
	...
	const selectedSuboptions = branchPath.filter(
		bc => bc.level > foundFromBranchPath?.level
	);
	const isSuboptionSelected = selectedSuboptions.length > 0;
	const isClickable =
		!isOptSelected || (isOptSelected && isSuboptionSelected);
	const cursorMaybe = isClickable
		? { cursor: "pointer" }
		: { cursor: "default" };
	...
	const optionName = [...ancestors, { label: optionLabelRaw }]
		.map(bc => bc.label)
		.join("/");
	const pathMatch =
		isOptSelected && ancestors.length + 1 === branchPath.length
			? "exact"
			: "nested";
	const ariaLabel = isOptSelected
		...
		: intl.formatMessage(
				{ id: "FieldSelectTree.screenreader.option" },
				{ optionName, pathMatch }
		  );
		...
		switch (e.key) {
			...
			case "ArrowRight": {
				if (hasSuboptions(config) && !isOptSelected) {
					...
					setTimeout(() => {
						const allOptions = getAllFocusableOptions(
							containerRef.current
						);
			...
			case "ArrowLeft": {
				...
				const parentButton = getParentOptionButton(
					e.target,
					containerRef.current
				);
				...
				if (level >= 1 && ancestors.length >= 0) {
					...
					if (branchPath.length > 0) {
						...
						const updatedValue = branchPath.reduce(
							(picked, selectedOptionByLevel) => {
								const {
									level: branchPathLevel,
									levelKey,
								} = selectedOptionByLevel;
								if (
									branchPathLevel <= parentLevel &&
									levelKey
								) {
									return {
										...picked,
										[levelKey]:
											selectedOptionByLevel.option,
									};
								}
								return picked;
							},
			...
			case "Home": {
				e.preventDefault();
				e.stopPropagation();
				const firstOption = getAllFocusableOptions(
					containerRef.current
				)?.[0];
				if (firstOption) {
					firstOption.focus();
	...
	return (
		<li
			...
			style={{ paddingLeft: `${12}px`, ...cursorMaybe }}
		>
			<button
...
const getBranchPath = (primaryOptions, currentSelections) => {
	const currentSelectionsEntries = Object.entries(currentSelections).sort(
		([k, v]) => k
	);
	return currentSelectionsEntries.reduce((branchPath, entry) => {
		...
		const currentOptions =
			branchPath.length > 0
				? branchPath[branchPath.length - 1].suboptions
				: primaryOptions;
		const foundOption = currentOptions.find(
			o => o.option === selectedOption
		);
		const foundOptionWithLevelKeyMaybe = foundOption
			? [{ ...foundOption, levelKey }]
			: [];
...
const handleChangeFn = (
	...
	const updatedCurrentBranchPath = branchPath.reduce(
		(picked, selectedOptionByLevel) => {
			const {
				level: branchPathLevel,
				option: optionByLevel,
			} = selectedOptionByLevel;
	...
	const isUpdateANewBranchPath =
		updatedCurrentBranchPath[levelKeyToUpdate] == null;
