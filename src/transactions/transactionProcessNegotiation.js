...
export const transitions = {
	...
	REQUEST_PAYMENT_TO_ACCEPT_OFFER:
	...
	OPERATOR_MARK_CHANGES_DELIVERED:
...
export const isNegotiationState = state => {
	...
	const unprefixedState =
		state.indexOf("/") === -1 ? state : state.split("/")[1];
	return [states.OFFER_PENDING, states.CUSTOMER_OFFER_PENDING].includes(
		unprefixedState
	);
};
...
export const getTransitionsWithMatchingOffers = (transitions, offers) => {
	const isValidOffersArray = isValidNegotiationOffersArray(
		transitions,
		offers
	);
		...
		for (let i = 0; i < transitions.length; i++) {
			...
			if (offerTransitions.includes(transition.transition)) {
				transition.offerInSubunits =
					offers[offerIndex]?.offerInSubunits;
