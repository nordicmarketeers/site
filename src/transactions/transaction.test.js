...
describe("transaction utils for default-purchase", () => {
	...
	const txHasBeenReceived = tx =>
		process.hasPassedState(process.states.RECEIVED, tx);
...
describe("transaction utils for default-booking", () => {
	...
	describe("tx has passed a state X", () => {
		const txHasBeenAccepted = tx =>
			process.hasPassedState(process.states.ACCEPTED, tx);
		const txHasBeenDelivered = tx =>
			process.hasPassedState(process.states.DELIVERED, tx);
...
describe("transaction utils for ConditionalResolver", () => {
	...
	it("exact parameter match succeeds", () => {
		...
		const resolvedOutputData = new ConditionalResolver(inputData)
			...
			.resolve();
		expect(resolvedOutputData).toEqual({
			showInfoX: true,
			isSomethingOn: true,
		});
	});
	...
	it("partial parameters (wildcard) match succeeds", () => {
		...
		const resolvedOutputData = new ConditionalResolver(inputData)
			.cond(["inquiry", "provider"], () => ({
				showInfoX: true,
				isSomethingOn: true,
			}))
			.cond(["purchase", "customer"], () => ({
				showInfoX: false,
				isSomethingOn: true,
			}))
			.cond(["purchase", _], () => ({
...
describe("getTransitionsWithMatchingOffers", () => {
	...
	const createNegotiationTransition = (
		transition,
		by,
		createdAt = new Date()
	) => ({
	...
	describe("with valid offers array", () => {
		...
		it("should preserve all transition properties", () => {
			...
			expect(result[0].transition).toBe(
				negotiationTransitions.MAKE_OFFER
			);
...
describe("isNegotiationState", () => {
	...
	describe("with unprefixed states", () => {
		it("should return true for OFFER_PENDING state", () => {
			const result = process.isNegotiationState(
				negotiationStates.OFFER_PENDING
			);
			expect(result).toBe(true);
		});
		...
		it("should return true for CUSTOMER_OFFER_PENDING state", () => {
			const result = process.isNegotiationState(
				negotiationStates.CUSTOMER_OFFER_PENDING
			);
			expect(result).toBe(true);
		});

		it("should return false for other states", () => {
			const result1 = process.isNegotiationState(
				negotiationStates.INITIAL
			);
			const result2 = process.isNegotiationState(
				negotiationStates.INQUIRY
			);
			const result3 = process.isNegotiationState(
				negotiationStates.QUOTE_REQUESTED
			);
			const result4 = process.isNegotiationState(
				negotiationStates.OFFER_REJECTED
			);
			const result5 = process.isNegotiationState(
				negotiationStates.PENDING_PAYMENT
			);
			const result6 = process.isNegotiationState(
				negotiationStates.PAYMENT_EXPIRED
			);
			const result7 = process.isNegotiationState(
				negotiationStates.OFFER_ACCEPTED
			);
			const result8 = process.isNegotiationState(
				negotiationStates.CANCELED
			);
			const result9 = process.isNegotiationState(
				negotiationStates.DELIVERED
			);
			const result10 = process.isNegotiationState(
				negotiationStates.CHANGES_REQUESTED
			);
			const result11 = process.isNegotiationState(
				negotiationStates.COMPLETED
			);
			const result12 = process.isNegotiationState(
				negotiationStates.REVIEWED_BY_CUSTOMER
			);
			const result13 = process.isNegotiationState(
				negotiationStates.REVIEWED_BY_PROVIDER
			);
			const result14 = process.isNegotiationState(
				negotiationStates.REVIEWED
			);
	...
	describe("with prefixed states", () => {
		it("should return true for prefixed OFFER_PENDING state", () => {
			const result = process.isNegotiationState(
				`state/${negotiationStates.OFFER_PENDING}`
			);
			expect(result).toBe(true);
		});
		...
		it("should return false for prefixed non-negotiation states", () => {
			const result1 = process.isNegotiationState(
				`state/${negotiationStates.INITIAL}`
			);
			const result2 = process.isNegotiationState(
				`state/${negotiationStates.COMPLETED}`
			);
			const result3 = process.isNegotiationState(
				`state/${negotiationStates.CANCELED}`
			);
