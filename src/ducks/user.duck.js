...
const fetchCurrentUserHasOrdersPayloadCreator = (
	_,
	{ getState, extra: sdk, rejectWithValue }
) => {
...
const fetchCurrentUserNotificationsPayloadCreator = (
	...
	const paramsForSales = {
		only: "sale",
		states: statesNeedingProviderAttention
	...
	const paramsForOrders = {
		only: "order",
		states: statesNeedingCustomerAttention
			.map(state => `state/${state}`)
			.join(","),
		page: 1,
		perPage: NOTIFICATION_PAGE_SIZE,
	...
	extraReducers: builder => {
		builder
			...
			.addCase(fetchCurrentUserThunk.fulfilled, (state, action) => {
				...
				state.currentUserShowTimestamp = action.payload
					? new Date().getTime()
					: 0;
			})
			...
			.addCase(
				fetchCurrentUserHasListingsThunk.fulfilled,
				(state, action) => {
					state.currentUserHasListings = action.payload.hasListings;
				}
			)
			.addCase(
				fetchCurrentUserHasListingsThunk.rejected,
				(state, action) => {
			...
			.addCase(
				fetchCurrentUserNotificationsThunk.fulfilled,
				(state, action) => {
					state.currentUserSaleNotificationCount =
						action.payload.saleNotificationsCount;
					state.currentUserOrderNotificationCount =
						action.payload.orderNotificationsCount;
				}
			)
			.addCase(
				fetchCurrentUserNotificationsThunk.rejected,
				(state, action) => {
					console.error(action.payload); // eslint-disable-line
			...
			.addCase(
				fetchCurrentUserHasOrdersThunk.fulfilled,
				(state, action) => {
					state.currentUserHasOrders = action.payload.hasOrders;
				}
			)
			.addCase(
				fetchCurrentUserHasOrdersThunk.rejected,
				(state, action) => {
					console.error(action.payload); // eslint-disable-line
