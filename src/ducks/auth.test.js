...
describe("auth duck", () => {
	describe("reducer", () => {
		...
		it("should login successfully", () => {
			...
			expect(
				authenticationInProgress({ auth: loginRequestState })
			).toEqual(true);
	...
	describe("login thunk", () => {
		it("should dispatch success and fetch current user", () => {
			...
			const fakeCurrentUserResponse = {
				data: { data: fakeCurrentUser, include: [] },
			};
			const fakeTransactionsResponse = {
				data: { data: [], include: [] },
			};
			const sdk = {
				...
				currentUser: {
					show: jest.fn(() =>
						Promise.resolve(fakeCurrentUserResponse)
					),
				},
				transactions: {
					query: jest.fn(() =>
						Promise.resolve(fakeTransactionsResponse)
					),
				},
			};
			let actions = [];
			...
			return login(username, password)(dispatch, getState, sdk).then(
				() => {
					expect(sdk.login.mock.calls).toEqual([
						[{ username, password }],
					]);
					...
					expect(actions[5].type).toEqual(
						"user/fetchCurrentUser/fulfilled"
					);
					expect(actions[5].payload).toEqual(fakeCurrentUser);

					expect(actions[6].type).toEqual(
						"user/fetchCurrentUserNotifications/fulfilled"
					);
		...
		it("should reject if logout is in progress", () => {
			const initialState = reducer(undefined, { type: "@@INIT" });
			const logoutInProgressState = reducer(initialState, {
