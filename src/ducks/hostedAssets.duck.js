				...
				(collectedAssets, assetEntry, i) => {
					...
					if (nonConfigAssets.includes(name)) {
						...
						return {
							...
							[name]: {
								path,
								data: getFirstAssetData(assetResponse),
							},
						};
					}
...
export const fetchAppAssets = (assets, version) => (
	dispatch,
	getState,
	sdk
) => {
...
// Wrapper functions to maintain the same API as the original thunks without Redux Toolkit.
export const fetchPageAssets = (assets, hasFallback) => (
	dispatch,
	getState,
	sdk
) => {
	...
	extraReducers: builder => {
		builder
			...
			.addCase(fetchPageAssetsThunk.pending, (state, action) => {
				state.currentPageAssets = action.meta.arg.assets
					? Object.keys(action.meta.arg.assets)
					: [];
				state.inProgress = true;
				state.error = null;
			})
