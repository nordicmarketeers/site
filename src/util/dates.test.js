...
describe("date utils", () => {
	...
	describe("isSameDate()", () => {
		...
		it("should be truthy if parameters match", () => {
			expect(
				isSameDate(new Date(2019, 0, 1), new Date(2019, 0, 1))
			).toBeTruthy();
		});
	});
	...
	describe("isInRange()", () => {
		...
		it("should succeed if date is same as inclusive end", () => {
			...
			expect(
				isInRange(inclusiveEnd, d1, d2, undefined, "Etc/UTC")
			).toBeTruthy();
		});
	...
	describe("getStartOf()", () => {
		...
		it("should find start of day in Europe/Helsinki", () => {
			const beginningOfDayInHelsinki = new Date(
				Date.UTC(2018, 11, 31, 22, 0, 0)
			);
			expect(getStartOf(date, "day", "Europe/Helsinki")).toEqual(
				beginningOfDayInHelsinki
			);
		});

		it("should find start of day with offset in Etc/UTC", () => {
			const twoDaysOff = new Date(Date.UTC(2019, 0, 3, 0, 0, 0));
			expect(getStartOf(date, "day", "Etc/UTC", 2, "days")).toEqual(
				twoDaysOff
			);
		});
		...
		it("should find start of previous month with offset in Etc/UTC", () => {
			...
			expect(getStartOf(date, "month", "Etc/UTC", -1, "months")).toEqual(
				prevMonth
			);
		});
	...
	describe("daysBetween()", () => {
		it("should fail if end date is before start date", () => {
			...
			expect(() => daysBetween(start, end)).toThrow(
	...
	describe("formatDateWithProximity()", () => {
		it("formats a date today", () => {
			...
			expect(
				formatDateWithProximity(d, intl, "Today", {
					timeZone: "Etc/UTC",
				})
			).toEqual("Today, 1:51 PM");
		});
	...
	describe("formatDateIntoPartials()", () => {
		it("formats a date into its partials", () => {
			const d = new Date(Date.UTC(2017, 10, 23, 13, 51));
			const partials = formatDateIntoPartials(d, intl, {
				timeZone: "Etc/UTC",
			});
	...
	describe("parseDateFromISO8601()", () => {
		...
		it("should read ISO 8601 date as date in Etc/UTC and return date and time in UTC formatted ISO 8601", () => {
			expect(
				parseDateFromISO8601("2020-04-07", "Etc/UTC").toISOString()
			).toEqual("2020-04-07T00:00:00.000Z");
		});
		...
		it("should read ISO 8601 date as date in Europe/Helsinki and return date and time in UTC formetted ISO 8601", () => {
			...
			).toEqual("2020-02-06T22:00:00.000Z");
		});
	});
	...
	describe("parseDateTimeString()", () => {
		...
		it("should read ISO 8601 date as date in Etc/UTC and return date and time in UTC formatted ISO 8601", () => {
			expect(
				parseDateTimeString("2020-04-07 01:00", "Etc/UTC").toISOString()
			).toEqual("2020-04-07T01:00:00.000Z");
		});
		...
		it("should read ISO 8601 date as date in Europe/Helsinki and return date and time in UTC formetted ISO 8601", () => {
			...
			).toEqual("2020-02-06T23:00:00.000Z");
		});
	});
	...
	describe("stringifyDateToISO8601()", () => {
		...
		it("should read given date in Europe/Helsinki and return ISO 8601 date string", () => {
			expect(
				stringifyDateToISO8601(
					new Date("2020-02-06T22:00:00.000Z"),
					"Europe/Helsinki"
				)
			).toEqual("2020-02-07");
		});
		...
		it("should read given date in America/New_York and return ISO 8601 date string", () => {
			...
			expect(stringifyDateToISO8601(date, "America/New_York")).toEqual(
				"2020-04-06"
			);
		});
	});
	...
	describe("findNextBoundary()", () => {
		it("should return 01:00 as next boundary when starting from 00:45", () => {
			expect(
				findNextBoundary(
					new Date("2025-02-02T00:45:00.000Z"),
					1,
					"hour",
					"Etc/UTC"
				)
		...
		it("should return 02:00 as next boundary when starting from 01:00", () => {
			...
			).toEqual(new Date("2025-02-02T02:00:00.000Z"));
		});
		...
		it("should return 07:00 as next boundary when starting from 6:15 (local time) and time unit is hour in Calcutta", () => {
			...
			expect(
				findNextBoundary(date_6_15_Calcutta, 1, "hour", "Asia/Calcutta")
			).toEqual(date_7_00_Calcutta);
		});
	...
	describe("getSharpHours()", () => {
		it("should return sharp hours, when startTime and endTime are sharp hours", () => {
			...
			expect(
				getSharpHours(startHour, endHour, "Europe/Helsinki", intl)
			).toEqual([
		...
		it("should return sharp hours, when startTime and endTime are half hours", () => {
			...
			expect(
				getSharpHours(startHour, endHour, "Europe/Helsinki", intl)
			).toEqual([{ timestamp: 1568797200000, timeOfDay: "12:00 PM" }]);
		});
	});
	...
	describe("getStartHours()", () => {
		it("should return sharp hours, when startTime and endTime are sharp hours", () => {
			...
			const endHour = new Date("2019-09-18T09:00:00.000Z");
			expect(
				getStartHours(startHour, endHour, "Europe/Helsinki", intl)
			).toEqual([{ timestamp: 1568793600000, timeOfDay: "11:00 AM" }]);
		});
		it("should return sharp hours, when startTime and endTime are half hours", () => {
			const startHour = new Date("2019-09-18T08:30:00.000Z");
			const endHour = new Date("2019-09-18T09:30:00.000Z");
	...
	describe("getEndHours()", () => {
		it("should return sharp hours, when startTime and endTime are sharp hours", () => {
			...
			const endHour = new Date("2019-09-18T09:00:00.000Z");
			expect(
				getEndHours(startHour, endHour, "Europe/Helsinki", intl)
			).toEqual([{ timestamp: 1568797200000, timeOfDay: "12:00 PM" }]);
		});
		it("should return sharp hours, when startTime and endTime are half hours", () => {
			const startHour = new Date("2019-09-18T08:30:00.000Z");
