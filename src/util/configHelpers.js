...
const getBuiltInCategorySpecs = (categories = []) => {
	...
	const maxDepth = depthFirstSearch(
...
const validAccessControl = accessControlConfig => {
	...
	return {
		...accessControl,
		marketplace: { private: false, ...marketplace },
	};
};
...
const validSchemaOptions = (enumOptions, schemaType) => {
	...
	const isValid =
		!isEnumSchemaType ||
		(shouldHaveSchemaOptions && arrayContainsOptionShapes);
			...
			(acc, entry) => {
				...
				const [isValid, prop] =
					name === "key"
						...
						? validListingTypesForBuiltInSetup(
								value,
								listingTypesInUse
						  )
				...
				const hasFoundValid = !(
					acc.isValid === false || isValid === false
				);
	...
	return (
		hostedListingFields?.map(listingField => {
			...
			const enumOptionsMaybe = ["enum", "multi-enum"].includes(schemaType)
				? { enumOptions }
				: {};
			const numberConfigMaybe =
				schemaType === "long" ? { numberConfig } : {};
...
const mergeSearchConfig = (
	...
	const listingTypeFilterMaybe =
		typeof listingTypeFilter?.enabled === "boolean"
			? [listingTypeFilter]
			: [];
...
export const mergeConfig = (configAsset = {}, defaultConfigs = {}) => {
	...
	const categoryConfiguration = getBuiltInCategorySpecs(
