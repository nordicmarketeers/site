...
export const zwspAroundSpecialCharsSplit = (wordToBreak, breakChars = "/") => {
	...
	const escapedBCArray = bcArray.map(c =>
		c.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
	);
	...
	return wordToBreak
		.split(reSplit)
		.map(w => (bcArray.includes(w) ? `${zwsp}${w}${zwsp}` : w));
};
	...
	if (parenthesesCount < 0) {
		const trailingParentheses = Array.from(str)
			...
			.reduce((count, currentChar) => {
				return currentChar === ")" && parenthesesCount + count < 0
					? ++count
					: count;
			}, 0);
	...
	if (word.match(urlRegex)) {
		...
		return word.split(urlRegex).map(w => {
			...
			const [sanitizedURL, extra] =
				!isEmptyString && linkify
					? splitExtraParentheses(sanitizeUrl(w))
					: [w];
...
export const richText = (text, options) => {
	...
	return text.split(nonWhiteSpaceSequence).reduce((acc, nextChild, i) => {
		const parts = flow([
			...
			v =>
				flatMap(v, w =>
					zwspAroundSpecialCharsSplit(w, breakCharsConfig)
				),
			v =>
				map(v, (w, j) =>
					wrapLongWord(w, `${i}${j}`, {
