...
describe("data utils", () => {
	describe("combinedRelationships()", () => {
		...
		it("takes existings rels if no new rels are given", () => {
			...
			expect(combinedRelationships(listingRels, undefined)).toEqual(
				listingRels
			);
		});
		...
		it("takes new rels if no existing rels are given", () => {
			...
			expect(combinedRelationships(undefined, listingRels)).toEqual(
				listingRels
			);
		});
		...
		it("merges two nonempty rels", () => {
			const authorRel = {
				data: { id: new UUID("author-id"), type: "author" },
			};
			const imagesRel = {
				data: [
					{ id: new UUID("image1"), type: "image" },
			...
			const newImagesRel = {
				data: { id: new UUID("image1"), type: "image" },
			};
			const reviewsRel = {
				data: [
			...
			const newListingRels = {
				images: newImagesRel,
				reviews: reviewsRel,
			};
			expect(
				combinedRelationships(oldListingRels, newListingRels)
			).toEqual({
				author: authorRel,
				images: newImagesRel,
				reviews: reviewsRel,
	...
	describe("combinedResourceObjects()", () => {
		...
		it("keeps old attributes if new does't have any", () => {
			const res1Attributes = {
				title: "Listing 1 title",
				description: "Listing 1 description",
			};
			const res2Relationships = {
				author: { data: { id: new UUID("author1"), type: "author" } },
			};
	...
	describe("updatedEntities()", () => {
		...
		it("handles a more complex merge", () => {
			...
			const listing2Updated = {
				...
				type: "listing",
				attributes: {
					title: "New listing 2 title",
					description: "new listing 2 description",
				},
	...
	describe("denormalisedEntities()", () => {
		const createListing = id => ({
			...
			attributes: {
				...
				description: `Listing ${id} description`,
			},
		});
		...
		it("returns no results with empty resources", () => {
			const entities = {
				listing: { listing1: createListing("listing1") },
			};
		...
		it("returns the selected entities", () => {
			...
			expect(denormalisedEntities(entities, resources)).toEqual([
				listing1,
				listing3,
			]);
		});
		...
		it("returns entities of different types", () => {
			...
			expect(denormalisedEntities(entities, [listing1, user1])).toEqual([
				listing1,
				user1,
			]);
		});
		...
		it("throws with circular relationships", () => {
			...
			const listing1WithRelationships = {
				...listing1,
				relationships: listing1Relationships,
			};
			const user1Relationships = { topListing: { data: listing1 } };
			const user1WithRelationships = {
				...user1,
				relationships: user1Relationships,
			};
			const entities = {
		...
		it("denormalises simple relationships", () => {
			...
			expect(denormalisedEntities(entities, [listing1])).toEqual([
		...
		it("denormalises multiple relationships", () => {
			...
			const entities = {
				listing: {
					listing1: listing1WithRelationships,
					listing2,
					listing3,
				},
				user: { user1 },
			...
			expect(
				denormalisedEntities(entities, [listing1, listing2])
			).toEqual([
		...
		it("denormalises multiple relationships when relationship data is empty", () => {
			...
			expect(
				denormalisedEntities(entities, [listing1, listing2])
			).toEqual([{ ...listing1, author: null, images: [] }, listing2]);
		});
	});
});
...
describe("humanizeLineItemCode", () => {
	it("should humanize a line item code", () => {
		expect(humanizeLineItemCode("line-item/new-line-item")).toEqual(
			"New line item"
		);
	});
...
describe("denormalizeAssetData", () => {
	...
	it("should deep clone asset without image references", () => {
		...
		expect(JSON.stringify(denormalizeAssetData(jsonObj))).toEqual(
