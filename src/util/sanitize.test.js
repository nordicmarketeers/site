...
describe("sanitize utils", () => {
	...
	describe("sanitizeUrl", () => {
		...
		it("does not alter http URLs with ports with alphanumeric characters", () => {
			expect(
				sanitizeUrl("http://example.com:4567/path/to:something")
		...
		it("does not alter https URLs with ports with alphanumeric characters", () => {
			expect(
				sanitizeUrl("https://example.com:4567/path/to:something")
		...
		it("does not alter deep-link urls with alphanumeric characters", () => {
			// Customized test
			expect(sanitizeUrl("com.blaa.demo://example")).toBe(
				"com.blaa.demo://example"
			);
		});

		it("does not alter mailto urls with alphanumeric characters", () => {
			expect(
				sanitizeUrl("mailto:test@example.com?subject=hello+world")
			...
			describe.each(["javascript", "data", "vbscript"])(
				...
				protocol => {
					it(`replaces ${protocol} urls with about:blank`, () => {
						expect(
							sanitizeUrl(`${protocol}:alert(document.domain)`)
						).toBe("about:blank");
					});
					...
					it(`does not replace ${protocol}: if it is not in the scheme of the URL`, () => {
						expect(
							sanitizeUrl(`http://example.com#${protocol}:foo`)
