...
describe("urlHelpers", () => {
	describe("parseFloatNum()", () => {
		...
		it("handles space characters", () => {
			expect(createSlug("ice hockey    tournament")).toEqual(
				"ice-hockey-tournament"
			);
		});

		it("handles special characters", () => {
			expect(createSlug("ice hockey!%/@$€ for the win")).toEqual(
		...
		it("handles umlauts", () => {
			expect(createSlug("jääkiekko / pesäpallo")).toEqual(
				"jaakiekko-pesapallo"
			);
		});
	...
	describe("LatLngBounds serialisation", () => {
		it("encodes and decodes", () => {
			...
			expect(decodeLatLngBounds(encodeLatLngBounds(bounds))).toEqual(
				bounds
			);
		});
	});
	...
	describe("stringify()", () => {
		...
		it("encodes values", () => {
			...
			expect(stringify(params)).toEqual(
				`bool=true&num=123&space=A${SPACE}and${SPACE}b`
			);
		});
