...
export const withViewport = Component => {
	...
	WithViewportComponent.displayName = `withViewport(${Component.displayName ||
		Component.name})`;
...
export const lazyLoadWithDimensions = (Component, options = {}) => {
	...
	class LazyLoadWithDimensionsComponent extends ReactComponent {
		...
		componentDidMount() {
			...
			this.defaultRenderTimeout = window.setTimeout(() => {
				if (this.isElementNearViewport(0)) {
					...
					const loadAfterInitialRendering =
						options.loadAfterInitialRendering;
		...
		handleWindowResize() {
			const shouldLoadToImproveScrolling =
				typeof options.loadAfterInitialRendering === "number";
			if (
				this.isElementNearViewport(NEAR_VIEWPORT_MARGIN) ||
		...
		isElementNearViewport(safetyBoundary) {
			if (this.element) {
				...
				return (
					(rect.top >= 0 &&
						rect.top <= viewportHeight + safetyBoundary) ||
					(rect.bottom >= -1 * safetyBoundary &&
