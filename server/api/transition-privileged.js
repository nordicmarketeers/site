...
const transactionPromise = (sdk, id) =>
	sdk.transactions.show({ id, include: ["listing"] });
...
const getUpdatedMetadata = (orderData, transition, existingMetadata) => {
	...
	return isNewOffer
		...
		? addOfferToMetadata(existingMetadata, {
				offerInSubunits: getAmountFromPreviousOffer(
...
module.exports = (req, res) => {
	...
	Promise.all([transactionPromise(sdk, bodyParams?.id), fetchCommission(sdk)])
		.then(responses => {
			...
			const listing = getListingRelationShip(
				showTransactionResponse.data
			);
			...
			metadataMaybe = getUpdatedMetadata(
				orderData,
				transitionName,
				existingMetadata
			);
		...
		.then(trustedSdk => {
			...
			if (isSpeculative) {
				return trustedSdk.transactions.transitionSpeculative(
					body,
					queryParams
				);
			}
