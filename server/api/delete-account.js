...
module.exports = (req, res) => {
	...
	Promise.all([
		...
		.then(responses => {
			if (
				hasOngoingTransactionsWithIncompletePaymentProcessing(responses)
			) {
		...
		.then(trustedSdk => {
			return trustedSdk.currentUser.delete({
				currentPassword,
		...
		.catch(e => {
			const options =
				e.message === HAS_INCOMPLETE_TRANSACTIONS
...
const hasOngoingTransactionsWithIncompletePaymentProcessing = responses => {
	...
	const combinedTransactions = responses.flatMap(
