...
const assetCdnBaseUrlMaybe = ASSET_CDN_BASE_URL
	? { assetCdnBaseUrl: ASSET_CDN_BASE_URL }
	: {};
...
const maxSockets = MAX_SOCKETS
	? parseInt(MAX_SOCKETS, 10)
	: MAX_SOCKETS_DEFAULT;
...
exports.fetchAccessControlAsset = sdk => {
	return sdk
		.assetsByAlias({
			paths: ["/general/access-control.json"],
			alias: "latest",
		})
		.then(response => {
			// Let's throw an error if we can't fetch branding for some reason
			...
			if (!accessControlAsset) {
				const message =
					"access-control configuration was not available.";
				const error = new Error(message);
				error.status = 404;
				error.statusText = message;
				error.data = {};
