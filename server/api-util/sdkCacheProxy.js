...
const TTL_ASSETS_BY_VERSION = pickTTL(
	process.env.TTL_ASSETS_BY_VERSION,
	24 * 60 * 60
);
...
const sdkEndpointCacheConfig = {
	...
	"sdk.assetByVersion": args => {
		...
		const cacheKey = argsStr
			? `${MARKETPLACE_ROOT_URL}#sdk.assetByVersion(${argsStr})`
			: null;
		return {
			key: cacheKey,
	...
	"sdk.assetsByAlias": args => {
		...
		const argsModified = isAboutAppCdnAssets
			? { paths: ["<appCdnAssets>"], alias }
			: args[0];
		const argsStr = JSON.stringify(argsModified);
		const cacheKey = args?.[0]
			? `${MARKETPLACE_ROOT_URL}#sdk.assetsByAlias(${argsStr})`
			: null;
		return {
			key: cacheKey,
			ttl: TTL,
	...
	const createProxy = (sdkPartial, path) => {
		return new Proxy(sdkPartial, {
			...
			get(target, property, receiver) {
				...
				const isCacheable = CACHEABLE_SDK_ENDPOINTS.includes(
					endPointPath
				);
