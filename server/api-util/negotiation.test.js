	...
	describe("isIntentionToMakeOffer(offerInSubunits, transitionName)", () => {
		describe("valid make offer transitions", () => {
			it("should return true for make-offer transition with positive offer", () => {
				expect(
					isIntentionToMakeOffer(1000, "transition/make-offer")
				).toBe(true);
				expect(
					isIntentionToMakeOffer(5000, "transition/make-offer")
				).toBe(true);
				expect(isIntentionToMakeOffer(1, "transition/make-offer")).toBe(
					true
				);
			});
			...
			it("should return false for make offer transitions with zero offer", () => {
				expect(isIntentionToMakeOffer(0, "transition/make-offer")).toBe(
					false
				);
				expect(
					isIntentionToMakeOffer(
			...
			it("should return false for make offer transitions with negative offer", () => {
				expect(
					isIntentionToMakeOffer(-100, "transition/make-offer")
				).toBe(false);
				expect(
					isIntentionToMakeOffer(
						-5000,
		...
		describe("invalid transitions", () => {
			...
			it("should return false for other transitions with positive offer", () => {
				expect(
					isIntentionToMakeOffer(1000, "transition/accept-offer")
				).toBe(false);
				expect(
					isIntentionToMakeOffer(5000, "transition/reject-offer")
				).toBe(false);
				expect(
					isIntentionToMakeOffer(
						1000,
						"transition/cancel-negotiation"
					)
				).toBe(false);
				expect(
					isIntentionToMakeOffer(
						5000,
						"transition/complete-negotiation"
					)
				).toBe(false);
			});
			...
			it("should return false for invalid transitions with zero offer", () => {
				...
				expect(
					isIntentionToMakeOffer(0, "transition/accept-offer")
				).toBe(false);
				expect(
					isIntentionToMakeOffer(0, "transition/invalid-transition")
				).toBe(false);
			});
			...
			it("should return false for invalid transitions with negative offer", () => {
				...
				expect(
					isIntentionToMakeOffer(-5000, "transition/accept-offer")
				).toBe(false);
				expect(
					isIntentionToMakeOffer(-1, "transition/invalid-transition")
				).toBe(false);
			});
		});
		...
		describe("edge cases", () => {
			...
			it("should handle malformed transition names", () => {
				...
				expect(
					isIntentionToMakeOffer(1000, "transition/make_offer")
				).toBe(false);
				expect(
					isIntentionToMakeOffer(1000, "transition/makeoffer")
				).toBe(false);
				expect(
					isIntentionToMakeOffer(1000, "TRANSITION/MAKE-OFFER")
				).toBe(false);
	...
	describe("isIntentionToMakeCounterOffer(offerInSubunits, transitionName)", () => {
		...
		describe("invalid transitions", () => {
			it("should return false for make offer transitions with positive offer", () => {
				expect(
					isIntentionToMakeCounterOffer(1000, "transition/make-offer")
				).toBe(false);
				expect(
					isIntentionToMakeCounterOffer(
						5000,
			...
			it("should return false for invalid transitions with zero offer", () => {
				expect(
					isIntentionToMakeCounterOffer(0, "transition/make-offer")
				).toBe(false);
				expect(
					isIntentionToMakeCounterOffer(0, "transition/accept-offer")
				).toBe(false);
				expect(
					isIntentionToMakeCounterOffer(
						0,
			...
			it("should return false for invalid transitions with negative offer", () => {
				expect(
					isIntentionToMakeCounterOffer(-100, "transition/make-offer")
				).toBe(false);
				expect(
					isIntentionToMakeCounterOffer(
						-5000,
		...
		describe("edge cases", () => {
			...
			it("should handle malformed transition names", () => {
				...
				expect(
					isIntentionToMakeCounterOffer(
						1000,
						"transition/customer_make_counter_offer"
					)
				).toBe(false);
				expect(
					isIntentionToMakeCounterOffer(
						1000,
						"transition/customermakecounteroffer"
					)
				).toBe(false);
				expect(
					isIntentionToMakeCounterOffer(
	...
	describe("isIntentionToRevokeCounterOffer(transitionName)", () => {
		...
		describe("invalid transitions", () => {
			it("should return false for make offer transitions", () => {
				expect(
					isIntentionToRevokeCounterOffer("transition/make-offer")
				).toBe(false);
				expect(
					isIntentionToRevokeCounterOffer(
						"transition/make-offer-after-inquiry"
			...
			it("should return false for other transitions", () => {
				expect(
					isIntentionToRevokeCounterOffer("transition/accept-offer")
				).toBe(false);
				expect(
					isIntentionToRevokeCounterOffer("transition/reject-offer")
				).toBe(false);
				expect(
					isIntentionToRevokeCounterOffer(
						"transition/cancel-negotiation"
		...
		describe("edge cases", () => {
			it("should handle malformed transition names", () => {
				expect(
					isIntentionToRevokeCounterOffer(
						"customer-withdraw-counter-offer"
					)
				).toBe(false);
				expect(
					isIntentionToRevokeCounterOffer(
						"transition/customer_withdraw_counter_offer"
					)
				).toBe(false);
				expect(
					isIntentionToRevokeCounterOffer(
						"transition/customerwithdrawcounteroffer"
					)
				).toBe(false);
				expect(
					isIntentionToRevokeCounterOffer(
				...
				expect(
					isIntentionToRevokeCounterOffer(
						"transition/customer-withdraw-counter-offer-extra"
					)
				).toBe(false);
				expect(
					isIntentionToRevokeCounterOffer(
						"transition/customer-withdraw"
					)
				).toBe(false);
			});
			...
			it("should throw error with correct error properties", () => {
				...
				try {
					...
					expect(error.message).toBe(
						"Past negotiation offers are invalid"
					);
