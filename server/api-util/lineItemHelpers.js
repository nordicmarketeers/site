...
const calculateCommissionWithPercentage = (percentage, amount) => {
	const hasValidAmountAndPercentage =
		amount != null && percentage != null && percentage > 0;
...
exports.calculateShippingFee = (
	...
	if (
		...
		const oneItemFee = getAmountAsDecimalJS(
		...
		const numericShippingFee = convertDecimalJSToNumber(
			oneItemFee.plus(additionalItemsTotal)
		);
...
exports.getProviderCommissionMaybe = (providerCommission, order, currency) => {
	...
	const hasCommissionPercentage = this.hasCommissionPercentage(
		providerCommission
	);
