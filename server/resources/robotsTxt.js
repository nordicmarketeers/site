...
const sendRobotsTxt = (req, res, robotsTxtPath) => {
	const sitemapIndexUrl = `${getRootURL({
		useDevApiServerPort: true,
	})}/sitemap-index.xml`;
	...
	try {
		const modifiedStream = new Transform({
			transform(chunk, encoding, next) {
				...
				let modifiedChunk = chunk
					.toString()
					.replace(
						/https:\/\/my\.marketplace\.com\/sitemap-index\.xml/g,
						sitemapIndexUrl
					);
		...
		const readStream = fs.createReadStream(robotsTxtPath, {
			encoding: "utf8",
		});
