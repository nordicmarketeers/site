...
const checkEnvVariables = variables => {
	const missingEnvVariables = variables.filter(v =>
		isEmpty(process.env?.[v])
	);
		...
		.then(html => {
			...
			if (dev) {
				...
				const debugData = {
					...
					context,
					loadDataMs:
						res.locals.timestampAfterLoadData -
						res.locals.beforeLoadDataTimestamp,
					renderingMs:
						res.locals.timestampAfterRender -
						res.locals.timestampAfterLoadData,
				};
				console.log(
